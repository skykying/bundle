...
import org.eclipse.team.internal.target.ui.ExportWizard;
...
public class SFTPExportWizard extends ExportWizard{
  ...
  protected void initializeDialogSettings(){
    ...
    IDialogSettings workbenchSettings=SFTPPlugin.getPlugin()
        .getDialogSettings();
    IDialogSettings section=workbenchSettings.getSection("SFTPExportWizard");//$NON-NLS-1$
    if(section==null){
      section=workbenchSettings.addNewSection("SFTPExportWizard");//$NON-NLS-1$
    }
  ...
  public void init(IWorkbench workbench, IStructuredSelection currentSelection){
    ...
    setWindowTitle(Policy.bind("SFTPExportWizard.5")); //$NON-NLS-1$
  }
  ...
  protected SubscriberParticipant createParticipant(ISynchronizeScope scope){
    return new SFTPSynchronizeParticipant(new ResourceScope(new IResource[0]));
  }
}
