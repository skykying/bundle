 ...
 *******************************************************************************/
package com.lembed.lite.studio.device.ui.tree;
...
public class AdvisedCellLabelProvider extends StyledCellLabelProvider {
	...
	protected final IColumnAdvisor columnAdvisor; // column advisor necessary
													// processing cell
	...
	protected void paint(Event event, Object element) {
		...
		if (columnAdvisor.hasSuffixButton(element, columnIndex)) {
			Rectangle buttonBounds = columnAdvisor.getSuffixButtonBounds(getCellBounds(event), element, columnIndex);
	...
	protected void drawImage(Event event, Object element) {
		...
		if (image != null) {
			...
			event.gc.setAlpha(ALPHA);
			event.gc.drawImage(image, imageBounds.x, imageBounds.y, imageBounds.width, imageBounds.height, dstX, dstY,
	...
	protected void drawButton(Event event, Object element) {
		...
		event.gc.fillGradientRectangle(cellBounds.x + 1, cellBounds.y + 1, cellBounds.width - 1, cellBounds.height,
				true);
		event.gc.setForeground(ColorConstants.COLOR_BORDER);
	 ...
	 *            {@GC} to draw in
	 * @param bounds
	 *            rectangle defining destination image position
	...
	protected int drawButtonImage(Image image, GC gc, Rectangle bounds) {
		...
		if (image != null) {
			...
			// gc.setAlpha(ALPHA);
			gc.drawImage(image, imageBounds.x, imageBounds.y, imageBounds.width, imageBounds.height, dstX, dstY,
					imageBounds.width, imageBounds.height);
			// gc.setAlpha(255);
	...
	protected void drawUrl(Event event, Object element) {
		...
		if (text != null) {
			...
			Point extent = event.gc.textExtent(text);
			event.gc.drawLine(x - 1, y + extent.y, x + extent.x - 1, y + extent.y);
		}
	}
	...
	public void update(ViewerCell cell) {
		...
		case COMBO:
			...
			if (s != null && !s.isEmpty()) {
				// workaround: append space to the text to avoid last character
				// truncation the cells
				s += ' ';
			}
			cell.setText(s);
			if (!enabled) {
