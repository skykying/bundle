...
public class LaunchGenerator {
	 ...
	 * 
	 * @param generator
	 *            {@link ICpGenerator} to launch
	 * @param configInfo
	 *            {@link ICpConfigurationInfo} to expand argument strings
	 */
	 ...
	 * 
	 * @param type
	 *            generator type to launch, if null tries to launch first
	 *            successful
	 */
	protected void launch(String type) {
		for (String launchType : CmsisConstants.LAUNCH_TYPES) {
			...
			try {
				...
				CpPlugIn.getDefault().emitLiteEvent(LiteEvent.GPDSC_LAUNCH_ERROR, e.getMessage());
				continue;
			} catch (NoSuchMethodException e) {
				CpPlugIn.getDefault().emitLiteEvent(LiteEvent.GPDSC_LAUNCH_ERROR,
