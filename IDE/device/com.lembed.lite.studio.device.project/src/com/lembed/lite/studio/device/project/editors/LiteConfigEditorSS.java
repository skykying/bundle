...
import com.lembed.lite.studio.device.config.IHelpContextIds;
import com.lembed.lite.studio.device.config.Messages;
import com.lembed.lite.studio.device.config.model.IConfigWizardItem;
import com.lembed.lite.studio.device.config.model.IConfigWizardItem.EItemErrorType;
import com.lembed.lite.studio.device.config.model.IConfigWizardItem.EItemType;
...
public class LiteConfigEditorSS extends MultiPageEditorPart implements IResourceChangeListener , ICEditor,ICModelBasedEditor,ISelectionChangedListener, ICReconcilingListener{
	
	public final static String ID = "com.lembed.lite.studio.device.config.editors.ConfigEditor";
...
//
	...
	private IWorkbenchPage getDefaultPage() {
		IWorkbenchPage page = PlatformUI.getWorkbench().getActiveWorkbenchWindow().getActivePage();
		
		return page;
	}
	
	private void openDefaultEditor(IWorkbenchPage page,IFile file) throws PartInitException {
		String editorId = null;
		IEditorRegistry register = PlatformUI.getWorkbench().getEditorRegistry();
		if(register != null) {
			IEditorDescriptor desc = register.getDefaultEditor(file.getName());
			if(desc != null) {
				editorId = desc.getId();
			}
		}
				...
				if (type == IResource.FILE && kind == IResourceDelta.REMOVED
					...
					if ((flags & IResourceDelta.MOVED_TO) == IResourceDelta.MOVED_TO) {
						...
						Display.getDefault().asyncExec(() -> {
							LiteConfigEditorSS.this.getEditorSite().getPage().closeEditor(LiteConfigEditorSS.this, true);
						});
						return false;
					}
				}
	...
	private class ConfigColumnAdvisor extends ColumnAdvisor {
		...
		public String getString(Object obj, int columnIndex) {
			...
			long realValue = modifyValueR(value, op, modifier, item.getMaxValue(), item.getMinValue());
		...
		public  long modifyValueR(long value, char op, long modifier, long max, long min) {
		...
		public long getCurrentSelectedIndex(Object element, int columnIndex) {
			...
			long modifier = item.getModifier();
			char op = item.getModification();
			long realValue = modifyValueR(item.getValue(), op, modifier,	item.getMaxValue(), item.getMinValue());
			return realValue;
		}
	...
	public void showHighlightRangeOnly(boolean showHighlightRangeOnly) {
		editor.showsHighlightRangeOnly();
	...
	public ITranslationUnit getTranslationUnit() {
		if(editor instanceof LiteCSConfigEditor) {
			return ((LiteCSConfigEditor) editor).getTranslationUnit();
		}
		// TODO Auto-generated method stub
		return null;
	}
	...
	public void aboutToBeReconciled() {
		if(editor instanceof LiteCSConfigEditor) {
			((LiteCSConfigEditor) editor).aboutToBeReconciled();
		}
	}
	...
	public void reconciled(IASTTranslationUnit ast, boolean force, IProgressMonitor progressMonitor) {
		if(editor instanceof LiteCSConfigEditor) {
			((LiteCSConfigEditor) editor).reconciled(ast, force, progressMonitor);
		}
	}
	...
	public void selectionChanged(SelectionChangedEvent event) {
		if(editor instanceof LiteCSConfigEditor) {
			((LiteCSConfigEditor) editor).selectionChanged(event);
		}
	}
