...
public class CpPackUnpackJob extends CpPackJob {
	...
	protected IStatus run(IProgressMonitor monitor) {
		...
		fPackInstaller.jobFinished(fJobId, LiteEvent.PACK_INSTALL_JOB_FINISHED, fResult);
	...
	private boolean unzip(IProgressMonitor monitor) {
		...
		if (fDestPath.toFile().exists()) {
			...
			if (returnCode == IDialogConstants.OK_ID) {
				DeviceUIUtils.deleteFolderRecursive(fDestPath.toFile());
			} else {
		...
		try {
			...
			DeviceUIUtils.findPdscFiles(fDestPath.toFile(), pdscFiles, 1);
			...
			if (pack != null) {
				...
				DeviceUIUtils.copy(new File(pdscFile), downloadPath.append(pack.getId() + CmsisConstants.EXT_PDSC).toFile());
				return true;
			}
			DeviceUIUtils.deleteFolderRecursive(fDestPath.toFile());
