...
import com.lembed.lite.studio.device.ui.tree.ColorConstants;
...
public class PacksView extends PackInstallerView {
	 ...
	 */
	public static final String ID = "com.lembed.lite.studio.device.pack.ui.views.PacksView"; //$NON-NLS-1$
			...
			if ((element instanceof ICpPackFamily) && !(element instanceof ICpPackCollection)) {
				...
				for (ICpPack pack : packFamily.getPacks()) {
					if (pack.getPackState() == PackState.INSTALLED) {
						...
						if (urlItem == null || !DeviceUIUtils.isValidURL(urlItem.getText())) {
							bPackOffline = true;
						}
						break;
					}
