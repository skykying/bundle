...
package com.lembed.lite.studio.device.core.lite.configuration;
 ...
 */
public interface ILiteConfiguration extends IAdaptable, IEvaluationResult {
	 ...
	 * 
	 * @return map of files to add to project: project relative path to
	 *         ICpFileInfo
	 */
	Map<String, ICpFileInfo> getProjectSourceFiles();

	/**
	 * Returns ICpFileInfo associated with project file resource
	 * 
	 * @param fileName
	 *            project relative path of a file
	 ...
	 * 
	 * @param fileName
	 *            project relative path of a file (can contain *)
	 * @return ICpFileInfos if exists
	 */
	ICpFileInfo[] getProjectFileInfos(String fileName);

	/**
	 * Checks if file needs to be added to project (will appear in Project
	 * Explorer view)
	 * 
	 * @param fi
	 *            ICpFileInfo that represents file to check
	 * @return true if file is to be added to project
	 */
	boolean isSourceFileToProject(ICpFileInfo fi);
	 ...
	 */
	Collection<String> getLiteComponentsHCode();
	 ...
	 * 
	 * @return path to DFP installation directory or null if DFP is not
	 *         installed
	 */
	String getDfpPath();
	...
	/**
	 * Check if the configuration is valid - device and all components are
	 * resolved
	 * 
	...
	Map<String, ICpFileInfo> getScvdFiles();

	Map<String, EFileCategory> getProjectExtFiles();

	Map<String, String> getSymbols();

	Map<String, String> getProjectIncludeFiles();
