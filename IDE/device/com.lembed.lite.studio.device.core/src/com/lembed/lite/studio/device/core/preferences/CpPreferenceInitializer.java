...
package com.lembed.lite.studio.device.core.preferences;
...
import com.lembed.lite.studio.device.core.ICpPackRootProvider;

/**
 * Initializes CMSIS pack and lite preferences
 */
public class CpPreferenceInitializer extends AbstractPreferenceInitializer {
	...
	private static String defaultPackRoot = "device"; // $NON_NLS-1$
	...
	public void initializeDefaultPreferences() {
		...
		if (defaultPreferences.get(CpPlugIn.CMSIS_PACK_ROOT_PREFERENCE, CmsisConstants.EMPTY_STRING).isEmpty()) {
			...
			if (defaultValue.isEmpty()) {
				defaultValue = getLiteSTUDIOInstalledDir();
			}			
	...
	public static String getLiteSTUDIOInstalledDir() {
		...
		if(installFile.exists()) {			
			...
			if (parentFile.isDirectory()) {
				String vpath = parentFile.getPath() + File.separator + defaultPackRoot;
				log(vpath);
	...
	public static String getPackRoot() {
		...
		if (!defaultRoot.isEmpty()) {
			setPackRoot(defaultRoot); // synchronize preferences with external
										// provider
			return defaultRoot;
		}
	...
	public static String getDefaultCpRepository() {
		...
		attr.setAttribute(CmsisConstants.REPO_NAME, CmsisConstants.REPO_LEMBED);
		attr.setAttribute(CmsisConstants.REPO_URL, CmsisConstants.REPO_LEMBED_INDEX_URL);
