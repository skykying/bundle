...
import com.lembed.lite.studio.manager.analysis.editor.hex.HexEncoder;
...
import com.lembed.lite.studio.manager.analysis.editor.hex.events.CursorKeyListener;
...
public class HexEditorControl {
	...
	public HexEditorControl(final HexEditor hexEditor, final Composite parent) {
		...
		hexEncoder = new HexEncoder(parent);

		//
		// Get list of supported encodings
		//
		encodingTypes = hexEncoder.getEncodingTypes();
...
//				e.gc.setBackground(PropertyChangeListener.getColor(rgb));
	 ...
	 */
	private Stack<IHexEditorCommand> undolist = new Stack<>();
	private Stack<IHexEditorCommand> redolist = new Stack<>();
	...
	class ModifyCommand implements IHexEditorCommand {
        ...
        public boolean execute() {
			if (!core(HexEditorConstants.CELL_STATUS_CHANGED))
				return false;

			return row * HexEditorConstants.TABLE_NUM_DATA_COLUMNS + col < table.getBufferSize();
		}
		...
		private boolean core(int status) {
			...
			int dataIndex = row * HexEditorConstants.TABLE_NUM_DATA_COLUMNS + col;
	...
	class DeleteCommand implements IHexEditorCommand {
		...
		@Override
        public boolean execute() {
			if (row * HexEditorConstants.TABLE_NUM_DATA_COLUMNS + col >= table.getBufferSize())
				return false;
			...
			try {
				...
				MessageDialog.openError(parent.getShell(), HexEditorConstants.DIALOG_TITLE_ERROR, Messages.HexEditorControl_0 + e);
				return false;
			}
        ...
        public void revoke() {
			...
			table.setCellStatus(row, col, size, HexEditorConstants.CELL_STATUS_UNDO);
			table.updateVisibleTable(true, true, true);
			if (singleCommand) {
				table.select(row);
				cursor.setSelection(row, col + 1);
	...
	class InsertDataCommand implements IHexEditorCommand {
        ...
        public boolean execute() {
			...
			try {
				...
				MessageDialog.openError(parent.getShell(), HexEditorConstants.DIALOG_TITLE_ERROR, Messages.HexEditorControl_1 + e);
				return false;
			}
			table.setCellStatus(row, col, size, HexEditorConstants.CELL_STATUS_INSERTED);
			table.updateVisibleTable(true, true, true);
			return before != table.getBufferSize();
		}
	...
	class AppendDataCommand implements IHexEditorCommand {
		...
		AppendDataCommand(int dataSize, int dataValue) {
			...
			row = table.getBufferSize() / HexEditorConstants.TABLE_NUM_DATA_COLUMNS;
			col = table.getBufferSize() % HexEditorConstants.TABLE_NUM_DATA_COLUMNS;
		}
        ...
        public boolean execute() {
			...
			try {
				...
				MessageDialog.openError(parent.getShell(), HexEditorConstants.DIALOG_TITLE_ERROR, Messages.HexEditorControl_2 + e);
				return false;
			}
			table.setCellStatus(row, col, size, HexEditorConstants.CELL_STATUS_INSERTED);
			table.updateVisibleTable(true, true, true);
	...
	private abstract class CombinedCommand implements IHexEditorCommand {
		protected List<IHexEditorCommand> commands = new ArrayList<>();
	...
	private class FindReplaceCommand extends CombinedCommand {
        ...
        public boolean execute() {
			...
			if (findHex) {
				findBytes = HexUtils.string2bytes(findString);
			...
			if (replacement != null) {
				if (replaceHex) {
					replacementBytes = HexUtils.string2bytes(replacement);
				} else {
					try {
		...
		private int runSearchLoopWithProgress(final byte[] findBytes, final byte[] lowerCaseBytes, final byte[] replacementBytes) {
			...
			try {
				...
				HexManager.log(e);
				MessageDialog.openError(parent.getShell(), HexEditorConstants.DIALOG_TITLE_ERROR, Messages.HexEditorControl_4 + e.getTargetException());
...
//							findText.setBackground(PropertyChangeListener.getColor(found == null && matchCount == 0 ? new RGB(255, 0, 0) : new RGB(255, 255, 255)));
        ...
        public void revoke() {
			if (all) {
				try {
					...
					HexManager.log(e);
					MessageDialog.openError(parent.getShell(), HexEditorConstants.DIALOG_TITLE_ERROR, Messages.HexEditorControl_11 + e.getTargetException());
				} catch (InterruptedException e) {
					e.printStackTrace();
				}
			} else {
	...
	private class PasteCommand extends CombinedCommand {
        ...
        public boolean execute() {
			...
			if (p.getColumnIndex() < 0 || p.getColumnIndex() >= HexEditorConstants.TABLE_NUM_DATA_COLUMNS)
				return false;
	...
	public void copy() {
		...
		if (cp.getColumnIndex() < 0 || cp.getColumnIndex() >= HexEditorConstants.TABLE_NUM_DATA_COLUMNS)
			return;
	 ...
	 * @return Combo
	 */
	private static Combo getEncodingCombo(Composite parent, Vector<String> encodingTypes) {
	...
	private void encodingChanged() {
		...
		table.getTable().getColumn(HexEditorConstants.TABLE_NUM_COLUMNS - 1).setText(encoding);
	}
	...
	private static void disposeCommands(Stack<IHexEditorCommand> list) {
	 ...
	 */
	private void createTopPanel(final Composite composite) {
		//
		// Create top panel
		//
		findRuler = new Composite(composite, SWT.NONE);
		GridLayout layout = new GridLayout();
		layout.numColumns = 2;
...
//				text.setBackground(PropertyChangeListener.getColor(valid ? new RGB(255, 255, 255) : new RGB(255, 0, 0)));
	 ...
	 * @param fpcomp reference to a parent component
	 * @param encodeType vector with all supported encoding types
	 */
	 ...
	 * @param parent reference to a parent component
	 */
	private void createBottomPanel(final Composite pp) {
		//
		// Create bottom panel
		//
		Composite bottomPanel = new Composite(pp, SWT.NONE);
		GridLayout layout = new GridLayout();
		layout.numColumns = 1;
		layout.horizontalSpacing = 10;
		layout.verticalSpacing = 0;
	...
	private void createOffsetPanel(Composite composite) {
		//
		// Offset panel
		//
		Composite offsetPanel = new Composite(composite, SWT.NONE);
	...
	private void createValuePanel(Composite composite) {
		//
		// Value panel
		//
		Composite valuePanel = new Composite(composite, SWT.NONE);
	...
	private void createFileSizePanel(Composite composite) {
		//
		// FileSize panel
		//
		Composite fileSizePanel = new Composite(composite, SWT.NONE);
	 ...
	 * @param tcursor table cursor
	 * @param editor table editor
	 */
	private void addCursorListeners(final TableCursor tcursor, final ControlEditor editor) {
		//
		// Add key listener
		//
		cursorKeyListener = new CursorKeyListener(this, hexEditor, tcursor, editor);

		tcursor.addKeyListener(cursorKeyListener);

		//
		// Add selection listener
		//
		tcursor.addSelectionListener(new SelectionAdapter() {
			//
	 ...
	 */
	private static void createTableHeader(final HexTable table) {
		for (int i = 0; i < HexEditorConstants.TABLE_NUM_COLUMNS; i++) {
			...
			} else if (i >= 1 && i <= HexEditorConstants.TABLE_NUM_DATA_COLUMNS) {
	...
	private void popupMenuInsertData(int rowIndex, int columnIndex) {
		...
		// Display the dialog
		//
		AddDataDialog addDataDialog = new AddDataDialog(parent.getShell(), Messages.HexEditorControl_47, HexEditorConstants.DIALOG_TYPE_INSERT);
	...
	private void selectFindResult(boolean reset) {
		...
		if (reset && table.getCellStatus(p.getRowIndex(), p.getColumnIndex()) != HexEditorConstants.CELL_STATUS_FINDRESULT) {
			return;
		}

		for (int i = 0; i < findPrevStatus.length; i++) {
			int status = reset ? findPrevStatus[i] : HexEditorConstants.CELL_STATUS_FINDRESULT;
