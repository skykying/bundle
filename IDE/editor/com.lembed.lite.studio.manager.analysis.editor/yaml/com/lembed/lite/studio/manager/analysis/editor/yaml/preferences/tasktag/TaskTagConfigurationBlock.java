 ...
 ******************************************************************************/
package com.lembed.lite.studio.manager.analysis.editor.yaml.preferences.tasktag;
...
import com.lembed.lite.studio.manager.analysis.editor.yaml.TaskTagPreference;
    ...
    private void checkIfOptionsComplete(String[] allKeys) {
        for (int i= 0; i < allKeys.length; i++) {
            if (fPreferenceStore.getString(allKeys[i]) == null) {
            	EditorLog.logError("Preference option missing: " + allKeys[i] + " (" + this.getClass().getName() +')');  //$NON-NLS-1$//$NON-NLS-2$
            }
        }
    }
