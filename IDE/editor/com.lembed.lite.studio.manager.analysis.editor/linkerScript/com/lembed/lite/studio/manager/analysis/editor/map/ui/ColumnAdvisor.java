 ...
 ******************************************************************************/
package com.lembed.lite.studio.manager.analysis.editor.map.ui;
...
public abstract class ColumnAdvisor implements IColumnAdvisor {
	...
	protected static final Cursor CURSOR_HAND = Display.getCurrent().getSystemCursor(SWT.CURSOR_HAND); // hand
	// cursor
	// for
	// URL
	// text
	...
	/**
	 * Handling mouse over event to display hand cursor when necessary. Default
	 * usage is when cell contains an URL
	 *
	 * @param e
	 *            mouse event
	 */
	...
	/**
	 * Handling mouse up event to display hand cursor when necessary. Default
	 * usage is to opens URL for an URL control
	 *
	 * @param e
	 *            mouse event
	 */
	 ...
	 *
	 * @param cellBounds
	 *            cell's bounds excluding suffix button if it exists
	 * @param pt
	 *            the mouse's point
	 * @param element
	 *            cell's element
	 ...
	 *
	 * @param buttonBounds
	 *            the suffix button's bounds that the click happens
	 * @param pt
	 *            the mouse's point
	 * @param element
	 *            cell's element
	 ...
	 *            cell's column index
	 * @param pt
	 *            point to show menu if needed (in display coordinates)
	 */
	...
	public Image getCheckboxImage(Object obj, int columnIndex) {
		if (getCellControlType(obj, columnIndex) == CellControlType.INPLACE_CHECK
			...
			if (enabled) {
				image = check ? Activator.getImage(Activator.ICON_CHECKED)
				        : Activator.getImage(Activator.ICON_UNCHECKED);
			} else {
				image = check ? Activator.getImage(Activator.ICON_CHECKED_GREY)
				        : Activator.getImage(Activator.ICON_UNCHECKED_GREY);
			}
			return image;
		}
		return null;
