 ...
 *
 */
public class ElfEditorStyledLabelProvider extends StyledCellLabelProvider {
    ...
    public ElfEditorStyledLabelProvider( ColorManager colorManager ) {
    ...
    public void update(ViewerCell cell) {
        ...
        if( e instanceof ElfOutlineElement ){
        	ElfOutlineElement element = (ElfOutlineElement) e;
            ...
            if( showTags ){
                //styledString.append( " " + element.section.getName(), TAG_STYLER );
            }
     ...
     */
    private String elementLabel( ElfOutlineElement element ){
    ...
    public Image getImage(Object element) {
        ...
        if( element instanceof ElfOutlineElement ){
        	...
        	ElfOutlineElement segment = ( ElfOutlineElement ) element;
            if( segment.type == ElfOutlineElement.SECTION_ITEM ){
                return get( IMG_DOCUMENT );
            } else if( segment.type == ElfOutlineElement.LINKER_ITEM ){
                if( 0 == segment.children.size() ){
                    return get( IMG_MAPPINGSCALAR );
                } else {
                    return get( IMG_MAPPING );
                }
            } else if( segment.type == ElfOutlineElement.GROUP_ITEM ){
                if( 0 == segment.children.size() ){
