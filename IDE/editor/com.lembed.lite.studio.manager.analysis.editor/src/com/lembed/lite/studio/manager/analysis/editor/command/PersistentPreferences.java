...
public class PersistentPreferences {
	...
	public static final String  TOOLCHAIN_RELATIVE_PATH_KEY 	= "toolchain.path.relative";
	...
	public static final String  BUILD_TOOLS_RELATIVE_PATH_KEY 	= "buildTools.path.relative";
	private static final String BUILD_TOOLS_SEARCH_PATH_KEY 	= "buildTools.search.path";
	private static final String BUILD_TOOLS_SEARCH_PATH_OS_KEY 	= "buildTools.search.path.%s";
	...
	private static String getString(String key, String defaultValue, IProject project) {
		...
		String value = EclipseUtils.getPreferenceValueForId(CrossGccPlugin.PLUGIN_ID, key, null, project);
		...
		value = SharedDefaults.getInstance().getSharedDefaultsMap().get(CrossGccPlugin.PLUGIN_ID + "." + key);
	...
	private static String getEclipseString(String key, String defaultValue) {
		...
		Preferences preferences = ConfigurationScope.INSTANCE.getNode(CrossGccPlugin.PLUGIN_ID);
	...
	private static String getWorkspaceString(String key, String defaultValue) {
		...
		// Access the Eclipse scope
		Preferences preferences = InstanceScope.INSTANCE.getNode(CrossGccPlugin.PLUGIN_ID);
	...
	public static void flush() {
		...
		try {
			ConfigurationScope.INSTANCE.getNode(CrossGccPlugin.PLUGIN_ID).flush();
			InstanceScope.INSTANCE.getNode(CrossGccPlugin.PLUGIN_ID).flush();
	...
	private static void putProjectString(String key, String value, IProject project) {
		...
		// Access the Eclipse scope
		Preferences preferences = new ProjectScope(project).getNode(CrossGccPlugin.PLUGIN_ID);
		preferences.put(key, value);
	}
	...
	public static void flush(IProject project) {
		...
		try {
			new ProjectScope(project).getNode(CrossGccPlugin.PLUGIN_ID).flush();
		} catch (BackingStoreException e) {
			;
		}
	}
	...
	public static String getBuildToolsRelativePath(){
		return getString(BUILD_TOOLS_RELATIVE_PATH_KEY,"",null);
	}
	...
	public static String getToolchainRelativePath(){
		return getString(TOOLCHAIN_RELATIVE_PATH_KEY,"",null);
	}
}
