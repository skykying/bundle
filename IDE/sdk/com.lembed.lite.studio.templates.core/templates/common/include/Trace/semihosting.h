...
#ifndef __ARM_SEMIHOSTING_H_
#define __ARM_SEMIHOSTING_H_

/**
 * Semihosting operations definations.
 */
 ...
 * configuration cannot trigger HaedFault exceptions while the debugger is connected,
 * so we use an illegal op code, that will trigger an UsageFault exception.
 */
...
static inline int __attribute__ ((always_inline)) call_host(int reason, void* arg)
{
	...
	/*
	 * Accordingly to page 13-77 of ARM DUI 0040D other registers can also be clobbered.
	 * Some memory positions may also by changed by a system call, so they should not be
	 * kept in registers. Note: we are assuming the manual is right and Angel is
	 * repecting the APCS
	 */
	return value;
}
