...
/* @NO_INCLUDE_SRC static */
int gcov_exit_open_gcda_file(struct gcov_info *gi_ptr    /* @NO_AUTO_GCDA_CREATION struct gcov_filename *gf */)
{
...
#if IN_LIBGCOV
GCOV_LINKAGE void gcov_write_counter(gcov_type value)
{
...
GCOV_LINKAGE void gcov_write_tag_length(gcov_unsigned_t tag,
...
GCOV_LINKAGE void gcov_seek(gcov_position_t base)
{
	if (gcov_var.offset) {
		gcov_write_block(gcov_var.offset);
	}
...
/* @NO_INCLUDE_SRC static */
void gcov_histogram_merge( gcov_bucket_type *tgt_histo, gcov_bucket_type *src_histo)
{
		...
		while (tgt_num > 0 && !src_done) {
			...
			if (!src_num) {
				...
				if (src_i < 0) {
					...
					if (!tmp_histo[tgt_i].min_value || tgt_histo[tgt_i].min_value < tmp_histo[tgt_i].min_value){
						tmp_histo[tgt_i].min_value = tgt_histo[tgt_i].min_value;
					}

					while (--tgt_i >= 0) {
						tmp_histo[tgt_i].num_counters += tgt_histo[tgt_i].num_counters;
			...
			if (!tmp_histo[tmp_i].min_value || merge_min < tmp_histo[tmp_i].min_value){
	...
	gcov_nonruntime_assert(
	    tmp_i >= 0 && tmp_i < GCOV_HISTOGRAM_SIZE && tmp_histo[tmp_i].num_counters > 0);
