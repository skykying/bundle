...
GCOV_LINKAGE /* @NO_INCLUDE_SRC static */int gcov_exit_open_gcda_file(
...
static struct gcov_fn_buffer * free_fn_data(const struct gcov_info *gi_ptr,
		struct gcov_fn_buffer *buffer, unsigned limit) {
	struct gcov_fn_buffer *next;
	unsigned ix, n_ctr = 0;
...
static gcov_unsigned_t crc32_unsigned(gcov_unsigned_t crc32,
...
static void gcov_histogram_insert(gcov_bucket_type *histogram, gcov_type value) {
	unsigned i;
...
static void gcov_compute_histogram(struct gcov_info *list, struct gcov_summary *sum)
{
	struct gcov_info *gi_ptr;
	 ...
	 * ending.
	 */
	for (gi_ptr = list; gi_ptr && !(gi_ptr == list && break_loop_counter);
			gi_ptr = gi_ptr->next, break_loop_counter++) {
...
static
#endif
gcov_unsigned_t compute_summary(struct gcov_info *list,	struct gcov_summary *this_prg
	...
	for (gi_ptr = list; gi_ptr && !(gi_ptr == list && break_loop_counter);
		...
		*max_length = len;
		...
		for (f_ix = 0; (unsigned) f_ix != gi_ptr->n_functions; f_ix++) {
			...
			crc32 = crc32_unsigned(crc32, gfi_ptr ? gfi_ptr->lineno_checksum : 0);
			if (!gfi_ptr){
				continue;
			}
...
static void write_one_data(const struct gcov_info *gi_ptr,
		const struct gcov_summary *prg_p, const gcov_position_t eof_pos,
...
/* @NO_GCDA_FILE const char *filename, */
		int run_counted,
		...
		if (!cs_all->runs && cs_prg->runs) {
			...
			cs_all->sum_max = cs_prg->sum_max;
				...
				/* Don't compare the histograms, which may have slight
				 ...
				 merge.  */
				&& (cs_all->num != cs_prg->num || cs_all->runs != cs_prg->runs
						|| cs_all->sum_all != cs_prg->sum_all
						|| cs_all->run_max != cs_prg->run_max
						|| cs_all->sum_max != cs_prg->sum_max)) {
#ifndef ENABLE_GCOV
...
static void gcov_sort_icall_topn_counter(const struct gcov_ctr_info *counters) {
	int i;
	gcov_type *values;
	int n = counters->num;
...
static void gcov_sort_topn_counter_arrays(const struct gcov_info *gi_ptr) {
	unsigned int i;
	int f_ix;
	const struct gcov_fn_info *gfi_ptr;
	const struct gcov_ctr_info *ci_ptr;
...
static void dump_one_gcov(struct gcov_info *gi_ptr, /* @NO_GCDA_FILE struct gcov_filename *gf, */
unsigned run_counted, gcov_unsigned_t crc32, struct gcov_summary *all_prg,
	...
	error = merge_summary(/* @NO_GCDA_FILE gf->filename, */run_counted, gi_ptr,
			&prg, this_prg, crc32, all_prg);
	if (error == -1)
		goto read_fatal;
...
static
#endif
void gcov_do_dump(struct gcov_info *list, int run_counted) {
	struct gcov_info *gi_ptr;
#ifndef ENABLE_GCOV
	...
	crc32 = compute_summary(list,
			&this_prg /* @NO_GCDA_FILE , &gf.max_length*/);
	...
	for (gi_ptr = list; gi_ptr && !(gi_ptr == list && break_loop_counter);
			gi_ptr = gi_ptr->next, break_loop_counter++)
		dump_one_gcov(gi_ptr, /* @NO_GCDA_FILE &gf,*/run_counted, crc32,
				&all_prg, &this_prg);
...
struct gcov_master __gcov_master = {GCOV_VERSION, 0};
#endif
