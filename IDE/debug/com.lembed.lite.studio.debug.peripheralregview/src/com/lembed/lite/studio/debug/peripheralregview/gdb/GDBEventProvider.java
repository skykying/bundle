...
public class GDBEventProvider implements IDebugContextListener {
	...
	/**
	 * Dsf Session Listener, handles Suspend and Resume events -> fires
	 * debugevents
	 * 
	 * @author Eduard Bechtold, I IA AS R&D DH AMB 2
	 */
	public class SessionListener {
		...
		public void eventDispatched(final IResumedDMEvent e) {
			// DebugEvent event = new DebugEvent(new
			// DsfSessionDebugEvent(DsfSession.getSession(sessionID)),
			// DebugEvent.RESUME);
			// DebugPlugin.getDefault().fireDebugEventSet(new
			// DebugEvent[]{event});
		}
		...
		public void eventDispatched(final ISuspendedDMEvent e) {
			DebugEvent event = new DebugEvent(new DsfSessionDebugEvent(DsfSession.getSession(sessionID)),
					DebugEvent.SUSPEND);
			DebugPlugin.getDefault().fireDebugEventSet(new DebugEvent[] { event });
		}
	...
	public void debugContextChanged(DebugContextEvent event) {
		DebugEvent e = new DebugEvent(
				new DebugContextChangedDebugEvent(GDBSessionTranslator.getSession(event.getContext())),
				DebugEvent.CHANGE);
