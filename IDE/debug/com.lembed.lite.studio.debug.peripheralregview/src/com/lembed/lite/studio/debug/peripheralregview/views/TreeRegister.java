...
public class TreeRegister extends TreeParent {
	...
	public boolean isReadWrite() {
		return (getType().toUpperCase().equals("RW") || getType().toUpperCase().equals("RW1")
				|| getType().toUpperCase().equals("RW1C") || getType().toUpperCase().equals("RW1S")
				|| getType().toUpperCase().equals("RWH"));
	}

	public boolean isReadOnly() {
		return (getType().toUpperCase().equals("RO") || getType().toUpperCase().equals("RC")
				|| getType().toUpperCase().equals("R"));
	}

	public boolean isWriteOnly() {
		return (getType().toUpperCase().equals("WO") || getType().toUpperCase().equals("W")
				|| getType().toUpperCase().equals("W1C") || getType().toUpperCase().equals("W1S")
	...
	private void writeValue() {
		PeripheralRegView.GDBi.writeMemory(getRegisterAddress(), getValue(), getByteSize());
	}

	/**
	 * Write value to the device
	 * 
	 * (needed for WriteOnly Registers... they ever return 0x00000000 on
	 * getValue())
	 */
	private void writeValue(long value) {
		PeripheralRegView.GDBi.writeMemory(getRegisterAddress(), value, getByteSize());
	}
	...
	public void readValue() {
		if (retrievalActive && !isWriteOnly()) {
			...
			if (parent instanceof TreeRegisterGroup) {
				...
				if (context != null) {
					value = PeripheralRegView.GDBi.readMemory(registerAddress, getByteSize());
				} else
					value = -1;
			} else
