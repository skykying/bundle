...
import com.lembed.lite.studio.debug.peripheralregview.gdb.IGDBInterfaceSuspendListener;
import com.lembed.lite.studio.debug.peripheralregview.gdb.IGDBInterfaceTerminateListener;
import com.lembed.lite.studio.debug.peripheralregview.parser.SVDXMLParser;
import com.lembed.lite.studio.debug.peripheralregview.preferences.PreferencePagePeripheralregview;
import com.lembed.lite.studio.debug.peripheralregview.preferences.PreferencePageEmbSysBehavior;
...
public class PeripheralRegView extends ViewPart implements IGDBInterfaceSuspendListener, IGDBInterfaceTerminateListener {
	...
	private Image selectedImage, unselectedImage, selectedFieldImage, unselectedFieldImage, infoImage,
	        interpretationImage, configButtonImage;
	static public IGDBInterface GDBi;
	...
	class ViewContentProvider implements IStructuredContentProvider, ITreeContentProvider {
		...
		public ViewContentProvider() {
			...
			getPreferenceStore().addPropertyChangeListener(new IPropertyChangeListener() {
				...
				public void propertyChange(PropertyChangeEvent event) {
					...
					if (event.getProperty().equals("architecture") || event.getProperty().equals("vendor")
					        || event.getProperty().equals("chip") || event.getProperty().equals("board")) {
						initialize();
		...
		private void initialize() {
			// initialize invisibleRoot with an empty TreeParent to show an
			// empty Tree if LoadXML fails
			invisibleRoot = new TreeParent("", "");
			try {
				invisibleRoot = new SVDXMLParser().LoadXML();
	...
	public void createPartControl(final Composite parent) {
		...
		configButton.addMouseListener(new MouseListener() {
			...
			public void mouseDown(MouseEvent e) {
				IPreferencePage page = new PreferencePagePeripheralregview();
				page.setTitle("PeripheralRegisterView");
		...
		column.setEditingSupport(new EditingSupport(viewer) {
			...
			protected boolean canEdit(Object element) {
				if (element instanceof TreeField && ((((TreeRegister) ((TreeField) element).getParent()).isReadWrite())
				...
				if (value instanceof String) {
					...
					if (element instanceof TreeField && ((String) value).startsWith("0x")) {
						...
						if (treeField.getValue() != -1 && treeField.getValue() != fvalue) {
							...
							// calculate register value + modified field to
							// write into register
							...
							rvalue = rvalue & (~mask); // clear field bits in
							// register value
							fvalue = fvalue << bitOffset; // shift field value
							// into its position
							// in the register
							fvalue = fvalue & mask; // just to be sure, cut
							// everything but the field
							rvalue = rvalue | fvalue; // blend the field value
							// into the register
							// value
		...
		column.setLabelProvider(new ColumnLabelProvider() {
			public String getText(Object element) {
				...
				if (element instanceof TreeRegister) {
					return Utils.longtoHexString(((TreeRegister) element).getRegisterAddress(), 32); // TODO:
				}
				/* get address	width from xml ... */
				if (element instanceof TreeField) {
					return "";
				} else {
					return "";
		...
		column.setLabelProvider(new CellLabelProvider() {
			...
			public String getToolTipText(Object element) {
				if (element instanceof TreeRegister) { // only display if more
					// than one line is
					// found
					if (com.lembed.lite.studio.debug.peripheralregview.views.Utils
			...
			public void update(ViewerCell cell) {
				...
				if (element instanceof TreeRegister) {
					cell.setText(com.lembed.lite.studio.debug.peripheralregview.views.Utils
