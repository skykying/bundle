...
public class GDBSessionTranslator {
	 ...
	 */
	static public int writeMemory(Object session, String address, String value, int iByteCount)
			throws TimeoutException {
		if (null == session)
			return -1;
	 ...
	 */
	public static int writeMemory(final DsfSession session, final String address, final String value, int iByteCount,
			int maximumTimeToWait) throws TimeoutException {
		if (!session.isActive())
			return -1;
		int ret = -1;
		...
		final ICommand<MIDataWriteMemoryInfo> info_wm = factory.createMIDataWriteMemory(fGdb.getContext(), 0, address,
				format, iByteCount, value);
	 ...
	 */
	public static long readMemory(final DsfSession session, final String address, int iByteCount, int maximumTimeToWait)
			throws TimeoutException {
		if (!session.isActive())
			return -1;
		long ret = -1;
		...
		if (null == fGdb)
			return -1;
		org.eclipse.cdt.dsf.mi.service.command.CommandFactory factory = fGdb.getCommandFactory();
		final ICommand<MIDataReadMemoryInfo> info_rm = factory.createMIDataReadMemory(fGdb.getContext(), 0, address,
				MIFormat.HEXADECIMAL, 1, 1, iByteCount, null);
		...
		Query<MIDataReadMemoryInfo> query = new Query<MIDataReadMemoryInfo>() {
			...
			protected void execute(final DataRequestMonitor<MIDataReadMemoryInfo> rm) {
				fGdb.queueCommand(info_rm, new DataRequestMonitor<MIDataReadMemoryInfo>(fGdb.getExecutor(), null) {
					@Override
					protected void handleCompleted() {
						rm.setData(getData());
						rm.done();
		...
		try {
			...
			if (data != null) {
				...
				if (endianessknown) {
					...
					// System.out.println("Unknown endianess:
					// "+info.getMIOutput().toString());
					ret = -1;
				}
			}
	...
	public static MIInfo postCLICommand(DsfSession session, String cLICommand, int maximumTimeToWait)
		...
		return executeQuery(info, "postCLICommand", "cLICommand=" + cLICommand, session, tracker, fGdb,
				maximumTimeToWait);
	 ...
	 *            to be executed
	 * @param functionName
	 *            of the function that calls executeQuery (for log purposes)
	 * @param details
	 *            of the function that calls executeQuery (for log purposes)
	 * @param session
	 *            the DsfSession in wich the command is called
	 * @param requestMonitor
	 *            created with the sessions executor
	 * @param fGdb
	 *            the IGDBControl executing the command
	 * @return the MIInfo result
	 * @throws TimeoutException
	 */
	static private MIInfo executeQuery(final ICommand<MIInfo> command, final String functionName, final String details,
		...
		try {
			...
			String message = functionName + " - failed, ErrorMessage: waiting time of " + maximumTimeToWait
					+ " ms passed";
			throw new TimeoutException(message);
		} finally {
			tracker.dispose();
