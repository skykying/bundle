...
public class GDBEventListener extends IGDBInterface implements IDebugEventSetListener {
	...
	public void handleDebugEvents(DebugEvent[] events) {
		for (DebugEvent event : events) {
			...
			if (currentSession != this.currentSession) { // Session has changed;
				...
				if (this.currentSession != null) { // new session is active and
													// known (Standard/DSF)
			...
			// source session should always be not null and the same as current
			// (do not handle events not from current selected session!)
