...
public class PreferencePagePeripheralregview extends PreferencePage implements IWorkbenchPreferencePage {
	Combo architecture;
	Combo vendor;
	// ImageCombo chip;
	Combo chip;
	...
	public PreferencePagePeripheralregview() {
		super();
	...
	private List<String> getDirList(String path, String pattern) {
		...
		Enumeration<URL> entries = Platform.getBundle("com.lembed.lite.studio.debug.peripheralregview.data").findEntries(path, pattern,
		                           false);
	...
	private void fillArchitecture() {
		for (String entry : getDirList("data", "*")) {
			if (!entry.equals("peripheralregview.dtd")) {
	...
	private void fillBoard(String selectedArchitecture, String selectedVendor, String selectedChip) {
		...
		Bundle bundle = Platform.getBundle("com.lembed.lite.studio.debug.peripheralregview.data");
		URL fileURL = bundle
		              .getEntry("data/" + selectedArchitecture + "/" + selectedVendor + "/" + selectedChip + ".xml");
		Document doc;
	...
	protected Control createContents(Composite parent) {
		...
		vendor.addSelectionListener(new SelectionAdapter() {
			public void widgetSelected(SelectionEvent event) {
				fillChip(architecture.getItem(architecture.getSelectionIndex()),
				         vendor.getItem(vendor.getSelectionIndex()));
			}
		});

		chip.addSelectionListener(new SelectionAdapter() {
			public void widgetSelected(SelectionEvent event) {
				fillBoard(architecture.getItem(architecture.getSelectionIndex()),
				          vendor.getItem(vendor.getSelectionIndex()), chip.getItem(chip.getSelectionIndex()));
			}
		});
