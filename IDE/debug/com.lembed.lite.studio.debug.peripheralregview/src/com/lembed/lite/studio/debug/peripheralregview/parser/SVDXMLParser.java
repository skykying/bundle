 ...
 *******************************************************************************/
package com.lembed.lite.studio.debug.peripheralregview.parser;
...
import com.lembed.lite.studio.debug.peripheralregview.views.Interpretations;
import com.lembed.lite.studio.debug.peripheralregview.views.TreeElement;
import com.lembed.lite.studio.debug.peripheralregview.views.TreeField;
import com.lembed.lite.studio.debug.peripheralregview.views.TreeGroup;
import com.lembed.lite.studio.debug.peripheralregview.views.TreeParent;
import com.lembed.lite.studio.debug.peripheralregview.views.TreeRegister;
import com.lembed.lite.studio.debug.peripheralregview.views.TreeRegisterGroup;

public class SVDXMLParser {
	private String store_board;

	private Document OpenXML() throws JDOMException, IOException {
		Bundle bundle = Platform.getBundle("com.lembed.lite.studio.debug.peripheralregview");
		IPreferenceStore store = Activator.getDefault().getPreferenceStore();
		String store_architecture = store.getString(SVDContant.SVD_ATTRIBUTE_ARCH_NAME);
		String store_vendor = store.getString(SVDContant.SVD_ATTRIBUTE_VENDOR_NAME);
		String store_chip = store.getString(SVDContant.SVD_ATTRIBUTE_CHIP_NAME);
		store_board = store.getString(SVDContant.SVD_ATTRIBUTE_BOARD_NAME);
	...
	public TreeParent LoadXML() throws ParseException, DataConversionException {
		...
		if (root.getName() == SVDContant.SVD_ATTRIBUTE_DEVICE_NAME) {
	...
	private TreeParent ParseSVD(Element root, TreeParent tempRoot) throws ParseException, DataConversionException {
		...
		Element peripherals = root.getChild(SVDContant.SVD_ATTRIBUTE_PERIPHERALS_NAME);
		if (peripherals != null) {
			List<Element> grouplist = peripherals.getChildren(SVDContant.SVD_ATTRIBUTE_PERIPHERAL_NAME);

			for (Element group : grouplist) {

				// Mandatory attribute name
				Element attr_rgname = group.getChild(SVDContant.SVD_ATTRIBUTE_NAME_NAME);
				String rgname;
				if (attr_rgname != null) {
					rgname = attr_rgname.getValue();
				} else {
					throw new ParseException("peripheral requires name", 1);//$NON-NLS-1
				}

				// Mandatory attribute baseAddress
				Element element_baseAddress = group.getChild("baseAddress");//$NON-NLS-1
				String baseAddress;
				...
				if (registers != null) {
					...
					for (Element register : registerlist) {
						...
						/////////////////////////////////////////////////////////////////////////////////////////////
						// TODO: << EST Handle <dim>, see
						///////////////////////////////////////////////////////////////////////////////////////////// http://www.keil.com/pack/doc/cmsis/SVD/html/group__dim_element_group__gr.html
						// <dim>4</dim>
						...
						if (rdim > 1) {
							Activator.log("DIM Register: " + rname + " " + rdescription + " " + lbaseAddress + " "
							              + raddress + " " + rresetvalue + " " + raccess + " " + rsize + " " + rdim + " "
							              + rdimIncrement + " " + rdimIndex);
						}
						int begIdx = 0, commaIdx = 0;
						Boolean isCommaList, isRangeList;

						isCommaList = rdimIndex.indexOf(',', 0) != -1; /* e.g. "0,1,2,3" */
						isRangeList = rdimIndex.indexOf('-', 0) != -1; /* e.g. "0-3" */ // \todo not handled
						// yet! */
						int rangeOffset = 0;
						if (isRangeList) {
						...
						for (int i = 0; i < rdim; i++) { // << EST
							if (rdim > 1) {
								...
								if (isCommaList) {
									if (i < rdim - 1) {
										commaIdx = rdimIndex.indexOf(',', begIdx);
										/**
										 * find position of ','
										 * in "1,2,3,4"
										 */
									} else {
										commaIdx = rdimIndex.length();
										/**
										 * there is no comma
										 * any more at the
										 * end of the list
									...
									/*
									 * If dimIndex is not specified, start
									 * with zero
									 */
									r = Integer.toString(i);
									begIdx = 0; /* reset index */
								}
								s = rname.replace("%s", r); // replace %s with
								// new string
								obj_register = new TreeRegister(s, rdescription, lbaseAddress + raddress, rresetvalue,
								                                raccess, rsize);
								lbaseAddress += rdimIncrement; // increment
								// register
							} else {
							...
							/////////////////////////////////////////////////////////////////////////////////////////////
							// old: // TreeRegister obj_register = new
							///////////////////////////////////////////////////////////////////////////////////////////// TreeRegister(rname,
							///////////////////////////////////////////////////////////////////////////////////////////// rdescription,
							///////////////////////////////////////////////////////////////////////////////////////////// lbaseAddress
							///////////////////////////////////////////////////////////////////////////////////////////// +
							...
							Activator.log("Register: " + rname + " " + rdescription + " " + lbaseAddress + " "
								...
								for (Element field : fieldlist) {
									...
									if (attr_fbitoffset != null) {
										...
										if (attr_fbitrange != null) {
											...
											fbitlength = (byte) (Byte.valueOf(rangeArray[0])
											                     - Byte.valueOf(rangeArray[1]) + 1);
										} else {
											...
											if (element_lsb != null && element_msb != null) {
												...
												fbitlength = (byte) (Byte.valueOf(element_msb.getValue())
												                     - Byte.valueOf(element_lsb.getValue()) + 1);
											} else
												throw new ParseException(
												    "field requires some sort of start-end bit marker", 1);
										}
									...
									if (enumeratedValues != null) {
										List<Element> interpretationlist = enumeratedValues
										                                   .getChildren("enumeratedValue");
									...
									Activator.log("Field: " + fname + " " + fdescription + " " + fbitoffset + " "
