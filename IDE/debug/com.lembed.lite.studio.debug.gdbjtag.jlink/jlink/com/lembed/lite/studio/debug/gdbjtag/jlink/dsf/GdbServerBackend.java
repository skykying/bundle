...
import com.lembed.lite.studio.debug.gdbjtag.jlink.SemihostingProcess;
...
public class GdbServerBackend extends GnuArmGdbServerBackend {
	...
	public GdbServerBackend(DsfSession session, ILaunchConfiguration lc) {
		...
		log("GdbServerBackend(" + session + "," + lc.getName() + ")");
	...
	public void initialize(final RequestMonitor rm) {
		...
		log("GdbServerBackend.initialize()");
	...
	private void doInitialize(RequestMonitor rm) {
		...
		log("GdbServerBackend.doInitialize()");
	...
	public void destroy() {
		...
		log("GdbServerBackend.destroy() " + Thread.currentThread());
	...
	/**
	 * Since the J-Link stderr messages are not final, this function makes the best
	 * use of the available information (the exit code and the captured string) to
	...
	protected class SemihostingStep extends InitializationShutdownStep {
		...
		public void initialize(final RequestMonitor rm) {
			...
			log("SemihostingStep.initialise()");

			if (fServerBackendState != State.STARTED) {

				log("SemihostingStep.initialise() skipped");
			...
			final RequestMonitor fTmpLaunchRequestMonitor = new RequestMonitor(getExecutor(), rm) {
				...
				protected void handleCompleted() {
					...
					log("SemihostingStep.initialise() handleCompleted()");
				...
				protected IStatus run(IProgressMonitor monitor) {
					...
					try {
						...
						int port = fLaunchConfiguration.getAttribute(
								CfgAttributes.GDB_SERVER_TELNET_PORT_NUMBER,
						...
						getExecutor().submit(new DsfRunnable() {
							...
							public void run() {
								...
								log("startSemihostingJob run State.STARTED");
					...
					log("startSemihostingJob run completed");
			...
			log("SemihostingStep.initialise() after job schedule");
				...
				public void run() {
					...
					if (!fSemihostingLaunchMonitor.fLaunched) {
						...
						if (jobThread != null) {
							...
							log("interrupt thread " + jobThread);
			...
			log("SemihostingStep.initialise() return");
		...
		protected void shutdown(final RequestMonitor requestMonitor) {
			...
			new Job(getTerminatingSemihostingJobName()) {
				...
				protected IStatus run(IProgressMonitor monitor) {
					try {
						...
						log("SemihostingStep.shutdown() run()");

						getExecutor().submit(new DsfRunnable() {
							@Override
							public void run() {

								log("SemihostingStep.shutdown() run() run()");
								...
								if (fSemihostingMonitorJob.fMonitorExited) {
									...
									log("SemihostingStep.shutdown() run() run() State.TERMINATED");
					...
					log("SemihostingStep shutdown() run() before getting exitValue");
					...
					while (attempts < 10) {
						try {
							...
							log("SemihostingStep shutdown() run() return");
			...
			log("SemihostingStep shutdown() return");
		...
		protected Process launchSemihostingProcess(String host, int port) throws CoreException {
			...
			log("launchSemihostingProcess() return " + proc);
	...
	private class SemihostingMonitorJob extends Job {
		...
		protected IStatus run(IProgressMonitor monitor) {
			synchronized (fProcess) {
				...
				log("SemihostingMonitorJob.run() submit " + fMonitorStarted + " thread " + getThread());
				...
				try {
					...
					getExecutor().submit(new DsfRunnable() {
						...
						public void run() {
							...
							log("SemihostingMonitorJob.run() run() thread " + getThread());

							// Destroy the entire backend
							destroy();

							log("SemihostingMonitorJob.run() run() State.TERMINATED");
				...
				log("SemihostingMonitorJob.run() fMonitorExited = true thread " + getThread());
			...
			synchronized (fProcess) {
				if (!fMonitorExited) {
					...
					if (thread != null) {
						...
						log("SemihostingMonitorJob.kill() interrupt " + thread.toString());
	...
	protected class SemihostingMonitorStep extends InitializationShutdownStep {
		...
		public void initialize(final RequestMonitor rm) {
			...
			log("SemihostingMonitorStep.initialize()");

			if (fServerBackendState != State.STARTED) {

				log("SemihostingMonitorStep.initialise() skipped");
		...
		protected void shutdown(RequestMonitor requestMonitor) {
			...
			log("SemihostingMonitorStep.shutdown()");
			...
			log("SemihostingMonitorStep.shutdown() return");
