...
/**
 * @since 7.0
 * https://mcuoneclipse.com/2014/11/15/tutorial-how-to-erase-the-flash-with-the-gnu-debugger/
 * add erease chip option to debugger table
 */
public class TabDebugger extends AbstractLaunchConfigurationTab {
	...
	private static final String TAB_ID = JlinkActivator.PLUGIN_ID + ".ui.debuggertab";
	...
	public void createControl(Composite parent) {
		...
		JlinkActivator.log("TabDebugger: createControl() ");
		...
		fUpdateThreadlistOnSuspend.setToolTipText(Messages.getString("DebuggerTab.update_thread_list_on_suspend_ToolTipText"));
	...
	private void createGdbServerGroup(Composite parent) {
...

		{
			...
			fDoGdbServerAllocateConsole.setToolTipText(Messages.getString("DebuggerTab.gdbServerAllocateConsole_ToolTipText"));
			gd = new GridData(GridData.FILL_HORIZONTAL);
	...
	public void initializeFrom(ILaunchConfiguration configuration) {
		...
		{
			JlinkActivator.log("TabDebugger: initializeFrom() " + configuration.getName());
		}
		...
		try {
			...
			// J-Link GDB Server Setup
			{
				...
				fDoStartGdbServer.setSelection(
				    configuration.getAttribute(CfgAttributes.DO_START_GDB_SERVER, booleanDefault));

				fDoConnectToRunning
				.setSelection(configuration.getAttribute(CfgAttributes.DO_CONNECT_TO_RUNNING,
				...
				fGdbServerExecutable.setText(
				    configuration.getAttribute(CfgAttributes.GDB_SERVER_EXECUTABLE, stringDefault));
				...
				String defaultDeviceName = configuration.getAttribute(CfgAttributes.FLASH_DEVICE_NAME_COMPAT,
				                           stringDefault);
				String deviceName = configuration.getAttribute(CfgAttributes.GDB_SERVER_DEVICE_NAME,
				                    defaultDeviceName);
				// Activator.log("got " + deviceName + " from: "
				// + configuration);
				fGdbFlashDeviceName.setText(deviceName);

				// Endianness
				stringDefault = PersistentPreferences.getGdbServerEndianness();
				String defaultEndianness = configuration.getAttribute(CfgAttributes.ENDIANNESS_COMPAT,
				                           stringDefault);
				String endianness = configuration.getAttribute(CfgAttributes.GDB_SERVER_DEVICE_ENDIANNESS,
				                    defaultEndianness);
				...
				String connection = configuration.getAttribute(CfgAttributes.GDB_SERVER_CONNECTION,
				                    stringDefault);
				...
				String connectionAddress = configuration
				                           .getAttribute(CfgAttributes.GDB_SERVER_CONNECTION_ADDRESS, stringDefault);
				fGdbServerConnectionAddress.setText(connectionAddress);

				// Interface
				stringDefault = PersistentPreferences.getGdbServerInterface();

				String defaultPhysicalInterface = configuration.getAttribute(CfgAttributes.INTERFACE_COMPAT,
				                                  stringDefault);
				String physicalInterface = configuration.getAttribute(CfgAttributes.GDB_SERVER_DEBUG_INTERFACE, defaultPhysicalInterface);
				...
				String defaultPhysicalInterfaceSpeed = configuration.getAttribute(CfgAttributes.GDB_SERVER_SPEED_COMPAT, stringDefault);

				String physicalInterfaceSpeed = configuration.getAttribute(CfgAttributes.GDB_SERVER_DEVICE_SPEED, defaultPhysicalInterfaceSpeed);
				...
				fGdbServerGdbPort.setText(
				    Integer.toString(configuration.getAttribute(CfgAttributes.GDB_SERVER_GDB_PORT_NUMBER,
				                     DefaultPreferences.GDB_SERVER_GDB_PORT_NUMBER_DEFAULT)));

				fGdbServerSwoPort.setText(
				    Integer.toString(configuration.getAttribute(CfgAttributes.GDB_SERVER_SWO_PORT_NUMBER,
				                     DefaultPreferences.GDB_SERVER_SWO_PORT_NUMBER_DEFAULT)));

				fGdbServerTelnetPort.setText(Integer
				                             .toString(configuration.getAttribute(CfgAttributes.GDB_SERVER_TELNET_PORT_NUMBER,
				                                       DefaultPreferences.GDB_SERVER_TELNET_PORT_NUMBER_DEFAULT)));

				// Flags
				fDoGdbServerVerifyDownload
				.setSelection(configuration.getAttribute(CfgAttributes.DO_GDB_SERVER_VERIFY_DOWNLOAD,
				              DefaultPreferences.DO_GDB_SERVER_VERIFY_DOWNLOAD_DEFAULT));

				fDoGdbServerInitRegs.setSelection(configuration.getAttribute(CfgAttributes.DO_GDB_SERVER_INIT_REGS,
				                                  DefaultPreferences.DO_GDB_SERVER_INIT_REGS_DEFAULT));

				fDoGdbServerLocalOnly.setSelection(configuration.getAttribute(CfgAttributes.DO_GDB_SERVER_LOCAL_ONLY,
				                                   DefaultPreferences.DO_GDB_SERVER_LOCAL_ONLY_DEFAULT));

				fDoGdbServerSilent.setSelection(configuration.getAttribute(CfgAttributes.DO_GDB_SERVER_SILENT,
				                                DefaultPreferences.DO_GDB_SERVER_SILENT_DEFAULT));

				// Log file
				fGdbServerLog.setText(configuration.getAttribute(CfgAttributes.GDB_SERVER_LOG,
				                      DefaultPreferences.GDB_SERVER_LOG_DEFAULT));

				// Other options
				stringDefault = PersistentPreferences.getGdbServerOtherOptions();
				fGdbServerOtherOptions.setText(configuration.getAttribute(CfgAttributes.GDB_SERVER_OTHER, stringDefault));

				// Allocate server console
				fDoGdbServerAllocateConsole.setSelection(configuration.getAttribute(CfgAttributes.DO_GDB_SERVER_ALLOCATE_CONSOLE,
				...
				fDoGdbServerAllocateSemihostingConsole.setSelection(
				    configuration.getAttribute(CfgAttributes.DO_GDB_SERVER_ALLOCATE_SEMIHOSTING_CONSOLE,
			...
			// GDB Client Setup
			{
				...
				fGdbClientOtherOptions.setText(
				    configuration.getAttribute(CfgAttributes.GDB_CLIENT_OTHER_OPTIONS, stringDefault));

				stringDefault = PersistentPreferences.getGdbClientCommands();
				fGdbClientOtherCommands.setText(configuration.getAttribute(CfgAttributes.GDB_CLIENT_OTHER_COMMANDS, stringDefault));
			}
		...
		JlinkActivator.log("TabDebugger: initializeFrom() completed " + configuration.getName());
	}

	public void initializeFromDefaults() {

		JlinkActivator.log("TabDebugger: initializeFromDefaults()");
	...
	public void activated(ILaunchConfigurationWorkingCopy workingCopy) {
		...
		JlinkActivator.log("TabDebugger: activated() " + workingCopy.getName());
	}

	@Override
	public void deactivated(ILaunchConfigurationWorkingCopy workingCopy) {
		
		JlinkActivator.log("TabDebugger: deactivated() " + workingCopy.getName());
	}

	@Override
	public void performApply(ILaunchConfigurationWorkingCopy configuration) {

		JlinkActivator.log("TabDebugger: performApply() " + configuration.getName() + ", dirty=" + isDirty());

		{
			// legacy definition; although the jtag device class is not used,
			// it must be there, to avoid NPEs
			configuration.setAttribute(IGDBJtagConstants.ATTR_JTAG_DEVICE, CfgAttributes.JTAG_DEVICE);
		}
		...
		// J-Link GDB server
		{
			...
			configuration.setAttribute(CfgAttributes.DO_START_GDB_SERVER, booleanValue);
			PersistentPreferences.putGdbServerDoStart(booleanValue);

			// Connect to running
			configuration.setAttribute(CfgAttributes.DO_CONNECT_TO_RUNNING,
			                           fDoConnectToRunning.getSelection());

			// Executable
			stringValue = fGdbServerExecutable.getText().trim();
			configuration.setAttribute(CfgAttributes.GDB_SERVER_EXECUTABLE, stringValue);
			PersistentPreferences.putGdbServerExecutable(stringValue);

			// Device name
			stringValue = fGdbFlashDeviceName.getText().trim();
			configuration.setAttribute(CfgAttributes.GDB_SERVER_DEVICE_NAME, stringValue);
			// Activator.log("stored " + stringValue + " to: "
			// + configuration);
			...
			configuration.setAttribute(CfgAttributes.GDB_SERVER_DEVICE_ENDIANNESS, stringValue);
			...
			configuration.setAttribute(CfgAttributes.GDB_SERVER_CONNECTION, stringValue);
			PersistentPreferences.putGdbServerConnection(stringValue);

			// Connection address
			stringValue = fGdbServerConnectionAddress.getText().trim();
			configuration.setAttribute(CfgAttributes.GDB_SERVER_CONNECTION_ADDRESS, stringValue);
			...
			configuration.setAttribute(CfgAttributes.GDB_SERVER_DEBUG_INTERFACE, stringValue);
			...
			configuration.setAttribute(CfgAttributes.GDB_SERVER_DEVICE_SPEED, stringValue);
			PersistentPreferences.putGdbServerInitialSpeed(stringValue);

			// Ports
			int port;
			port = Integer.parseInt(fGdbServerGdbPort.getText().trim());
			configuration.setAttribute(CfgAttributes.GDB_SERVER_GDB_PORT_NUMBER, port);

			port = Integer.parseInt(fGdbServerSwoPort.getText().trim());
			configuration.setAttribute(CfgAttributes.GDB_SERVER_SWO_PORT_NUMBER, port);

			port = Integer.parseInt(fGdbServerTelnetPort.getText().trim());
			configuration.setAttribute(CfgAttributes.GDB_SERVER_TELNET_PORT_NUMBER, port);

			// Flags
			configuration.setAttribute(CfgAttributes.DO_GDB_SERVER_VERIFY_DOWNLOAD,
			                           fDoGdbServerVerifyDownload.getSelection());

			configuration.setAttribute(CfgAttributes.DO_GDB_SERVER_INIT_REGS,
			                           fDoGdbServerInitRegs.getSelection());

			configuration.setAttribute(CfgAttributes.DO_GDB_SERVER_LOCAL_ONLY,
			                           fDoGdbServerLocalOnly.getSelection());

			configuration.setAttribute(CfgAttributes.DO_GDB_SERVER_SILENT, fDoGdbServerSilent.getSelection());

			// Log file
			configuration.setAttribute(CfgAttributes.GDB_SERVER_LOG, fGdbServerLog.getText().trim());

			// Other options
			stringValue = fGdbServerOtherOptions.getText().trim();
			configuration.setAttribute(CfgAttributes.GDB_SERVER_OTHER, stringValue);
			PersistentPreferences.putGdbServerOtherOptions(stringValue);

			// Allocate server console
			configuration.setAttribute(CfgAttributes.DO_GDB_SERVER_ALLOCATE_CONSOLE,
			                           fDoGdbServerAllocateConsole.getSelection());

			// Allocate semihosting console
			configuration.setAttribute(CfgAttributes.DO_GDB_SERVER_ALLOCATE_SEMIHOSTING_CONSOLE,
		...
		// GDB client
		{
			...
			configuration.setAttribute(CfgAttributes.GDB_CLIENT_OTHER_OPTIONS, stringValue);
			PersistentPreferences.putGdbClientOtherOptions(stringValue);

			stringValue = fGdbClientOtherCommands.getText().trim();
			configuration.setAttribute(CfgAttributes.GDB_CLIENT_OTHER_COMMANDS, stringValue);
		...
		JlinkActivator.log("TabDebugger: performApply() completed " + configuration.getName() + ", dirty=" + isDirty());
	}

	@Override
	public void setDefaults(ILaunchConfigurationWorkingCopy configuration) {

		JlinkActivator.log("TabDebugger: setDefaults() " + configuration.getName());
		...
		// J-Link GDB server setup
		{
			...
			configuration.setAttribute(CfgAttributes.DO_START_GDB_SERVER, defaultBoolean);

			configuration.setAttribute(CfgAttributes.DO_CONNECT_TO_RUNNING,
			                           DefaultPreferences.DO_CONNECT_TO_RUNNING_DEFAULT);

			defaultString = PersistentPreferences.getGdbServerExecutable();
			configuration.setAttribute(CfgAttributes.GDB_SERVER_EXECUTABLE, defaultString);
			...
			configuration.setAttribute(CfgAttributes.GDB_SERVER_DEVICE_NAME, sharedName);

			defaultString = PersistentPreferences.getGdbServerEndianness();
			configuration.setAttribute(CfgAttributes.GDB_SERVER_DEVICE_ENDIANNESS, defaultString);

			defaultString = PersistentPreferences.getGdbServerConnection();
			configuration.setAttribute(CfgAttributes.GDB_SERVER_CONNECTION, defaultString);

			defaultString = PersistentPreferences.getGdbServerConnectionAddress();
			configuration.setAttribute(CfgAttributes.GDB_SERVER_CONNECTION_ADDRESS, defaultString);

			defaultString = PersistentPreferences.getGdbServerInterface();
			configuration.setAttribute(CfgAttributes.GDB_SERVER_DEBUG_INTERFACE, defaultString);

			defaultString = PersistentPreferences.getGdbServerInitialSpeed();
			configuration.setAttribute(CfgAttributes.GDB_SERVER_DEVICE_SPEED, defaultString);

			configuration.setAttribute(CfgAttributes.GDB_SERVER_GDB_PORT_NUMBER,
			                           DefaultPreferences.GDB_SERVER_GDB_PORT_NUMBER_DEFAULT);

			configuration.setAttribute(CfgAttributes.GDB_SERVER_SWO_PORT_NUMBER,
			                           DefaultPreferences.GDB_SERVER_SWO_PORT_NUMBER_DEFAULT);

			configuration.setAttribute(CfgAttributes.GDB_SERVER_TELNET_PORT_NUMBER,
			                           DefaultPreferences.GDB_SERVER_TELNET_PORT_NUMBER_DEFAULT);
			...
			configuration.setAttribute(CfgAttributes.DO_GDB_SERVER_SILENT,
			                           DefaultPreferences.DO_GDB_SERVER_SILENT_DEFAULT);

			configuration.setAttribute(CfgAttributes.GDB_SERVER_LOG,
			                           DefaultPreferences.GDB_SERVER_LOG_DEFAULT);

			defaultString = PersistentPreferences.getGdbServerOtherOptions();
			configuration.setAttribute(CfgAttributes.GDB_SERVER_OTHER, defaultString);
		...
		// J-Link GDB client setup
		{
			...
			configuration.setAttribute(CfgAttributes.GDB_CLIENT_OTHER_OPTIONS,
			                           DefaultPreferences.getGdbClientOtherOptions());

			defaultString = PersistentPreferences.getGdbClientCommands();
			configuration.setAttribute(CfgAttributes.GDB_CLIENT_OTHER_COMMANDS, defaultString);
		}
