...
public class Configuration {
	...
	public static String getGdbServerCommand(ILaunchConfiguration configuration) {
		...
		try {
			if (!configuration.getAttribute(CfgAttributes.DO_START_GDB_SERVER,
					DefaultPreferences.getGdbServerDoStart())) {
				return null;
			}

			executable = configuration.getAttribute(CfgAttributes.GDB_SERVER_EXECUTABLE,
	...
	public static String[] getGdbServerCommandLineArray(ILaunchConfiguration configuration) {
		...
		try {
			...
			String connectionAddress = configuration.getAttribute(CfgAttributes.GDB_SERVER_CONNECTION_ADDRESS,
			...
			lst.add("-if");
			lst.add(configuration.getAttribute(CfgAttributes.GDB_SERVER_DEBUG_INTERFACE,
					DefaultPreferences.getGdbServerInterface()));
			String defaultName = configuration.getAttribute(CfgAttributes.FLASH_DEVICE_NAME_COMPAT,
					DefaultPreferences.FLASH_DEVICE_NAME_DEFAULT);

			String name = configuration.getAttribute(CfgAttributes.GDB_SERVER_DEVICE_NAME, defaultName)
					.trim();
			...
			lst.add("-endian");
			lst.add(configuration.getAttribute(CfgAttributes.GDB_SERVER_DEVICE_ENDIANNESS,
					DefaultPreferences.getGdbServerEndianness()));

			lst.add("-speed");
			lst.add(configuration.getAttribute(CfgAttributes.GDB_SERVER_DEVICE_SPEED,
					DefaultPreferences.getGdbServerInitialSpeed()));

			lst.add("-port");
			lst.add(Integer.toString(configuration.getAttribute(CfgAttributes.GDB_SERVER_GDB_PORT_NUMBER,
					DefaultPreferences.GDB_SERVER_GDB_PORT_NUMBER_DEFAULT)));

			lst.add("-swoport");
			lst.add(Integer.toString(configuration.getAttribute(CfgAttributes.GDB_SERVER_SWO_PORT_NUMBER,
					DefaultPreferences.GDB_SERVER_SWO_PORT_NUMBER_DEFAULT)));

			lst.add("-telnetport");
			lst.add(Integer.toString(configuration.getAttribute(CfgAttributes.GDB_SERVER_TELNET_PORT_NUMBER,
					DefaultPreferences.GDB_SERVER_TELNET_PORT_NUMBER_DEFAULT)));

			if (configuration.getAttribute(CfgAttributes.DO_GDB_SERVER_VERIFY_DOWNLOAD,
					DefaultPreferences.DO_GDB_SERVER_VERIFY_DOWNLOAD_DEFAULT)) {
				lst.add("-vd");
			}

			if (configuration.getAttribute(CfgAttributes.DO_CONNECT_TO_RUNNING,
					DefaultPreferences.DO_CONNECT_TO_RUNNING_DEFAULT)) {
				lst.add("-noreset");
				lst.add("-noir");
			} else {
				if (configuration.getAttribute(CfgAttributes.DO_GDB_SERVER_INIT_REGS,
			...
			if (configuration.getAttribute(CfgAttributes.DO_GDB_SERVER_LOCAL_ONLY,
			...
			if (configuration.getAttribute(CfgAttributes.DO_GDB_SERVER_SILENT,
					DefaultPreferences.DO_GDB_SERVER_SILENT_DEFAULT)) {
				lst.add("-silent");
			}

			String logFile = 
			configuration.getAttribute(CfgAttributes.GDB_SERVER_LOG, DefaultPreferences.GDB_SERVER_LOG_DEFAULT)
					.trim();
			...
			String other = configuration.getAttribute(CfgAttributes.GDB_SERVER_OTHER,
	...
	public static String getGdbServerDeviceName(ILaunchConfiguration config) throws CoreException {
		...
		return config.getAttribute(CfgAttributes.GDB_SERVER_DEVICE_NAME,
	...
	public static String[] getGdbClientCommandLineArray(ILaunchConfiguration configuration) {
		...
		String other;
		try {
			other = configuration.getAttribute(CfgAttributes.GDB_CLIENT_OTHER_OPTIONS,
	...
	public static boolean getDoStartGdbServer(ILaunchConfiguration config) throws CoreException {
		...
		return config.getAttribute(CfgAttributes.DO_START_GDB_SERVER,
				DefaultPreferences.getGdbServerDoStart());
	}
	...
	public static boolean getDoAddServerConsole(ILaunchConfiguration config) throws CoreException {
		...
		return getDoStartGdbServer(config)
				&& config.getAttribute(CfgAttributes.DO_GDB_SERVER_ALLOCATE_CONSOLE,
	...
	public static boolean getDoAddSemihostingConsole(ILaunchConfiguration config) throws CoreException {
		...
		return getDoStartGdbServer(config)
				&& config.getAttribute(CfgAttributes.DO_GDB_SERVER_ALLOCATE_SEMIHOSTING_CONSOLE,
