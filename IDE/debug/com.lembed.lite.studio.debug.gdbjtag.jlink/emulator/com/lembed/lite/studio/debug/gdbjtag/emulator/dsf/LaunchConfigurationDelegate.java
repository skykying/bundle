...
public class LaunchConfigurationDelegate extends AbstractGnuArmLaunchConfigurationDelegate {
	...
	protected void launchDebugSession(final ILaunchConfiguration config, ILaunch l, IProgressMonitor monitor)
		...
		if (LaunchUtils.getIsNonStopMode(config) && !isNonStopSupportedInGdbVersion(gdbVersion)) {
			cleanupLaunch(launch);
			throw new DebugException(new Status(IStatus.ERROR, DevicePlugin.PLUGIN_ID, DebugException.REQUEST_FAILED,
		...
		// Assign 4 work ticks.
		IProgressMonitor subMonServer = new SubProgressMonitor(monitor, 4,	SubProgressMonitor.PREPEND_MAIN_LABEL_TO_SUBTASK);
		...
		try {
			...
			throw new DebugException(new Status(IStatus.ERROR, DevicePlugin.PLUGIN_ID, DebugException.INTERNAL_ERROR,
					"Interrupted Exception in dispatch thread", e1)); //$NON-NLS-1$
		} catch (ExecutionException e1) {
			throw new DebugException(new Status(IStatus.ERROR, DevicePlugin.PLUGIN_ID, DebugException.REQUEST_FAILED,
					"Error in services launch sequence", e1.getCause())); //$NON-NLS-1$
	...
	protected IPath checkBinaryDetails(final ILaunchConfiguration config) throws CoreException {
		...
		if (boardName.isEmpty() && deviceName.isEmpty()) {
			throw new CoreException(new Status(IStatus.ERROR, DevicePlugin.PLUGIN_ID, "Missing mandatory board or device. "
