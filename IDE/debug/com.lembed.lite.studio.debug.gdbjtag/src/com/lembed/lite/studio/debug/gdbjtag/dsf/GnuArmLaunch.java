...
public class GnuArmLaunch extends GdbLaunch {
	...
	/**
	 * Instantiates a new gnu arm launch.
	 *
	...
	public void initialize() {
		...
		GdbActivator.log("GnuArmLaunch.initialize() " + this); //$NON-NLS-1$
		...
		try {
			...
			GdbActivator.log(new Status(IStatus.ERROR, GdbActivator.PLUGIN_ID, IDsfStatusConstants.INTERNAL_ERROR,
	...
	public void shutdownSession(final RequestMonitor rm) {
		...
		{
			GdbActivator.log("GnuArmLaunch.shutdownSession() " + this); //$NON-NLS-1$
		}
	...
	public void initializeControl() throws CoreException {
		...
		{
			GdbActivator.log("GnuArmLaunch.initializeControl()"); //$NON-NLS-1$
		}
		...
		try {
			...
			throw new CoreException(new Status(IStatus.ERROR, GdbActivator.PLUGIN_ID, 0,
			...
			Process cliProc = getDsfExecutor().submit(new Callable<Process>() {
				...
				public Process call() throws CoreException {
					...
					if (gdb != null) {
						...
						/**
						 * changed from getCLIProcess method to below for cdt update
						 * @2017.9.28
						 */
