 ...
 * "com.lembed.lite.studio.debug.core.gdbjtag.dsf.GnuArmLaunch" to provide
...
public class GnuArmAdapterFactory extends GdbAdapterFactory {
	...
	static void disposeAdapterSet(ILaunch launch) {
		...
		synchronized (fgLaunchAdaptersMap) {
			if (fgLaunchAdaptersMap.containsKey(launch)) {
			    SessionAdapters adapter = fgLaunchAdaptersMap.remove(launch);
			    if(adapter != null) {
			        adapter.dispose();
			    }
	...
	/**
	 * Instantiates a new gnu arm adapter factory.
	 */
	public GnuArmAdapterFactory() {
		DebugPlugin.getDefault().getLaunchManager().addLaunchListener(this);
		GdbActivator.log("GnuArmAdapterFactory !"); //$NON-NLS-1$
	}
