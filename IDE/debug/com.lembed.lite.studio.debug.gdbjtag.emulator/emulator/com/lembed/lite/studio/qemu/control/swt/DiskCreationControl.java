...
public class DiskCreationControl {
	...
	public DiskCreationControl(String sizeMB) {
		diskModel = new DiskCreationModel(sizeMB);
		diskModel.setDefault_virtual_machines_path("");
		diskView = new DiskCreationView();
		diskView.configureStandardMode();
	...
	public void showsOutput() throws IOException {
		...
		String line;
		try {
			iSReader = new InputStreamReader(diskModel.getMyprocess().getInputStream(), Charset.forName("UTF-8"));
	...
	public String getDiskImageSize() {
		return (Double.parseDouble(diskView.getEditor().getTextField().getText().replace(",", ".")) * 1024) + "M";
	}
	...
	public void setOptions() {
		if (diskView.getDiskExtension().getSelectedItem().equals(".img")) {
			...
			if (diskView.getEncryption_box().isSelected() || diskView.getPreallocation_metadata_box().isSelected()
		...
		} else if (diskView.getDiskExtension().getSelectedItem().equals(".vmdk")) {
			if (diskView.getCompat6_vmdk_box().isSelected() || diskView.getSubformat_vmdk_box().isSelected()) {
				List<String> options = new ArrayList<String>();
				...
				if (diskView.getSubformat_vmdk_box().isSelected()) {
					if (diskView.getSubformat_vmdk_combo().getSelectedItem().equals("monolithicSparse")) {
						options.add("subformat=monolithicSparse");
					} else if (diskView.getSubformat_vmdk_combo().getSelectedItem().equals("monolithicFlat")) {
						options.add("subformat=monolithicFlat");
					} else if (diskView.getSubformat_vmdk_combo().getSelectedItem().equals("twoGbMaxExtentSparse")) {
						options.add("subformat=twoGbMaxExtentSparse");
					} else if (diskView.getSubformat_vmdk_combo().getSelectedItem().equals("twoGbMaxExtentFlat")) {
						options.add("subformat=twoGbMaxExtentFlat");
					} else if (diskView.getSubformat_vmdk_combo().getSelectedItem().equals("streamOptimized")) {
		...
		} else if (diskView.getDiskExtension().getSelectedItem().equals(".vpc")) {
			if (diskView.getSubformat_vpc_box().isSelected()) {
				if (diskView.getSubformat_vpc_combo().getSelectedItem().equals("dynamic")) {
					diskModel.setOption("subformat=dynamic");
				} else if (diskView.getSubformat_vpc_combo().getSelectedItem().equals("fixed")) {
	...
	public void setOptionCreationNewVM(String diskExtension, List<String> options) {
		if (diskExtension.equals(".img")) {
			...
			if (options.contains("encryption") || options.contains("preallocation=metadata")
					|| options.contains("cluster_size=512k") || options.contains("cluster_size=1M")
			...
			if (options.contains("compat6") || options.contains("monolithicSparse")
					|| options.contains("monolithicFlat") || options.contains("twoGbMaxExtentSparse")
					|| options.contains("twoGbMaxExtentFlat") || options.contains("streamOptimized")) {
				diskModel.setOptions(options);
			} else {
				diskModel.unsetOption();
			}
		} else if (diskExtension.equals(".vpc")) {
			if (options.contains("subformat=dynamic") || options.contains("subformat=fixed")) {
				diskModel.setOptions(options);
			} else {
				diskModel.unsetOption();
			}
