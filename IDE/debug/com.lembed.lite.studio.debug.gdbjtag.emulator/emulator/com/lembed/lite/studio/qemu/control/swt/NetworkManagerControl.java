...
public class NetworkManagerControl implements BaseControl {
	...
	public NetworkManagerControl(EmulationControl myemulation, EmulatorQemuMachineControl myfile) {
		...
		for (int i = 0; i < NetworkManagerView.networkOptionsNumber; i++) {
			this.mycontrol[i] = new NetworkWorkerControl(myemulation, myfile, (i + 1));
			this.mycontrol[i].starts();
		}
		this.mymodel = new NetworkManagerModel(myemulation, myfile);
		if ((!mycontrol[0].isEnabled() && !mycontrol[1].isEnabled() && !mycontrol[2].isEnabled()
				&& !mycontrol[3].isEnabled() && !mycontrol[4].isEnabled() && !mycontrol[5].isEnabled()
				&& !mycontrol[6].isEnabled() && !mycontrol[7].isEnabled() && !mycontrol[8].isEnabled()
				&& !mycontrol[9].isEnabled())) {
