...
public class NetworkUserWorkerView extends DeviceViewWithFileChooser {
	...
	public NetworkUserWorkerView(EmulatorQemuMachineControl myfile, int position) {
		...
		hostnameDescription = new JLabel("Client hostname from built-in DHCP server:");
		...
		dhcpstartDescription = new JLabel("First of the 16 IPs the built-in DHCP server can assign:");
		...
		dnsDescription = new JLabel("Guess-visible address of the virtual nameserver:");
		...
		smbChooser = new JButton("Choose the root directory of the built-in SMB server:");
		...
		smbserverDescription = new JLabel("Choose the IP address of the built-in SMB server:");
		...
		guestfwd = new JButton("Forward guest TCP connections to the IP address server!");
	...
	private void buildMe(String optionString) {
		...
		for (String option : options) {
			if (option.startsWith("vlan=")) {
				...
				this.hostname.setText(option.substring(option.indexOf("=") + 1));
			} else if (option.startsWith("dhcpstart=")) {
				this.dhcpstart.setText(option.substring(option.indexOf("=") + 1));
