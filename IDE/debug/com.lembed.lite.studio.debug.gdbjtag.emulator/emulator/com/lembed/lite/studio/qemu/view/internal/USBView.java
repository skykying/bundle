...
public class USBView extends JFileChooserView {
    ...
    public USBView(FileControl myfile) {
        ...
        if (myfile.getFilemodel().getUsbMouseOption() != null) {
            if (myfile.getFilemodel().getUsbMouseOption().equals("true")) {
        ...
        if (myfile.getFilemodel().getUsbBrailleOption() != null) {
            if (myfile.getFilemodel().getUsbBrailleOption().equals("true")) {
                this.braille.setSelected(true);
            }
        }

        if (myfile.getFilemodel().getUsbDiskOption() != null) {
            if (!myfile.getFilemodel().getUsbDiskOption().isEmpty()) {
                this.disk.setSelected(true);
                this.file.setText(myfile.getFilemodel().getUsbDiskOption());
            }
        }
        ...
        if (myfile.getFilemodel().getUsbSerialOption() != null) {
            if (!myfile.getFilemodel().getUsbSerialOption().isEmpty()) {
                ...
                if (myfile.getFilemodel().getUsbSerialOption()
                        .contains("vendorid=")
                        || myfile.getFilemodel().getUsbSerialOption()
                        .contains("productid=")) {
                    if (myfile.getFilemodel().getUsbSerialOption()
                            .contains("vendorid=")
                            && !myfile.getFilemodel().getUsbSerialOption()
                            .contains("productid=")) {
                        if (myfile.getFilemodel().getUsbSerialOption().contains(":")) {
                            vendorid.setText(myfile.getFilemodel().getUsbSerialOption().substring(myfile.getFilemodel().getUsbSerialOption().indexOf("=") + 1,
                                    myfile.getFilemodel().getUsbSerialOption().indexOf(":")));
                            dev.setText(myfile.getFilemodel().getUsbSerialOption().substring(myfile.getFilemodel().getUsbSerialOption().indexOf(":") + 1));
                        } else {
                            vendorid.setText(myfile.getFilemodel().getUsbSerialOption().substring(myfile.getFilemodel().getUsbSerialOption().indexOf("=") + 1));
                        }

                    } else if (!myfile.getFilemodel().getUsbSerialOption()
                            .contains("vendorid=")
                            && myfile.getFilemodel().getUsbSerialOption()
                            .contains("productid=")) {
                        if (myfile.getFilemodel().getUsbSerialOption().contains(":")) {
                            productid.setText(myfile.getFilemodel().getUsbSerialOption().substring(myfile.getFilemodel().getUsbSerialOption().indexOf("=") + 1,
                                    myfile.getFilemodel().getUsbSerialOption().indexOf(":")));
                            dev.setText(myfile.getFilemodel().getUsbSerialOption().substring(myfile.getFilemodel().getUsbSerialOption().indexOf(":") + 1));
                        } else {
                            productid.setText(myfile.getFilemodel().getUsbSerialOption().substring(myfile.getFilemodel().getUsbSerialOption().indexOf("=") + 1));
                            ...
                            .contains("productid=")) {
                        if (myfile.getFilemodel().getUsbSerialOption().contains(":")) {
                            String work = myfile.getFilemodel().getUsbSerialOption().substring(0, myfile.getFilemodel().getUsbSerialOption().indexOf(":"));
                            ...
                            vendorid.setText(myfile.getFilemodel().getUsbSerialOption().substring(myfile.getFilemodel().getUsbSerialOption().indexOf("=") + 1,
                                    myfile.getFilemodel().getUsbSerialOption().indexOf(",")));
                            productid.setText(myfile.getFilemodel().getUsbSerialOption().substring(myfile.getFilemodel().getUsbSerialOption().indexOf(",") + 11));
                        }
                    }
                } else {
                    this.dev.setText(myfile.getFilemodel().getUsbSerialOption());
                }
            }
        }
        ...
        if (myfile.getFilemodel().getUsbNetOption() != null) {
            if (!myfile.getFilemodel().getUsbNetOption().isEmpty()) {
                ...
                String[] options = myfile.getFilemodel().getUsbNetOption().split(",");
