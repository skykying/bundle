...
public class BootView extends DeviceViewWithFileChooser {
	...
	private JLabel orderLabel, onceLabel, firstOrderLabel, secondOrderLabel, thirdOrderLabel, firstOnceLabel,
			secondOnceLabel, thirdOnceLabel, menuLabel, splashLabel, strictLabel;

	private JCheckBox splashTime, rebootTimeout;

	private JComboBox<String> firstOrder, secondOrder, thirdOrder, firstOnce, secondOnce, thirdOnce, menu, strict;
	...
	private String[] bootDevices = { "", "a - Floppy 1", "b - Floppy 2", "c - First Hard Disk", "d - First CDROM",
			"n - Network" };
	...
	public void resolveOrderOptions() {
		...
		String[] bootDevices = { "", "a - Floppy 1", "b - Floppy 2", "c - First Hard Disk", "d - First CDROM",
				"n - Network" };
	...
	public void applyView(IemultorStore store) {
		...
		if (eQControl.getMachineModel().getBootOnce1() != null) {
			for (int i = 0; i < bootDevices.length; i++) {
				if (!bootDevices[i].isEmpty()) {
					if (eQControl.getMachineModel().getBootOnce1().equals(bootDevices[i].substring(0, 1))) {
		...
		if (eQControl.getMachineModel().getBootOnce2() != null) {
			for (int i = 0; i < bootDevices.length; i++) {
				if (!bootDevices[i].isEmpty()) {
					if (eQControl.getMachineModel().getBootOnce2().equals(bootDevices[i].substring(0, 1))) {
		...
		if (eQControl.getMachineModel().getBootOnce3() != null) {
			for (int i = 0; i < bootDevices.length; i++) {
				if (!bootDevices[i].isEmpty()) {
					if (eQControl.getMachineModel().getBootOnce3().equals(bootDevices[i].substring(0, 1))) {
