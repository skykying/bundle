...
import com.lembed.lite.studio.qemu.model.swt.ConfigurationModel;
import com.lembed.lite.studio.qemu.model.swt.LastUsedFileEnumModel;
...
public class ConfigurationControl implements BaseControl {
	...
	public ConfigurationControl(LastUsedFolderModel myLastUsedFolderModel, LastUsedFileModel myLastUsedFileModel) {
		...
		this.fileControl = new EmulatorQemuMachineControl();

		if (Model.isValidString(this.lastUsedFileModel
				.getLastUsedFile(LastUsedFileEnumModel.LOADJAVAQEMUCONFIGURATIONFILE.getValor()))) {
			if (Model.isValidString(this.lastUsedFolderModel
					.getLastUsedFolder(LastUsedFolderEnumModel.OPENEXISTINGJAVAQEMUCONFIGURATION.getValor()))) {
				this.fileControl.getMachineModel()
						.readConfigurationFromXML(Model.combine(
				...
				this.fileControl.getMachineModel().getConfiguration(
				...
				this.configurationModel
						.setDefault_virtual_machines_path(this.fileControl.getMachineModel().getDefaultVMPath());
				this.configurationModel
						.setExecute_before_start_qemu(this.configurationView.getExecute_before_start_qemu_choices());
				this.configurationModel
						.setQemu_executable_path(this.configurationView.getQemu_executable_path_choice());
				this.configurationModel
						.setExecute_after_stop_qemu(this.configurationView.getExecute_after_stop_qemu_choices());
				this.configurationModel
						.setQemu_img_executable_path(this.configurationView.getQemu_img_executable_path_choice());
				this.configurationModel
						.setBios_vga_bios_keymaps_path(this.configurationView.getBios_vga_bios_keymaps_path_choice());
			}
		}
	...
	public void actionPerformed(ActionEvent e) {
		if (e.getActionCommand().equals("ExitCommand")) {
			...
			if (this.lastUsedFolderModel
					.getLastUsedFolder(LastUsedFolderEnumModel.SETDEFAULTVMDIRECTORY.getValor()) != null) {
				if (!this.lastUsedFolderModel
						.getLastUsedFolder(LastUsedFolderEnumModel.SETDEFAULTVMDIRECTORY.getValor()).equals(".")) {
					this.configurationView.setCurrentDirectory(this.lastUsedFolderModel
							.getLastUsedFolder(LastUsedFolderEnumModel.SETDEFAULTVMDIRECTORY.getValor()));
				} else if (this.configurationView.getDefault_virtual_machines_path_choice().getText().isEmpty()) {
					this.configurationView.setCurrentDirectory(".");
				} else {
					this.configurationView.setCurrentDirectory(
							this.configurationView.getDefault_virtual_machines_path_choice().getText());
				}
			} else if (this.configurationView.getDefault_virtual_machines_path_choice().getText().isEmpty()) {
				this.configurationView.setCurrentDirectory(".");
			} else {
			...
			if (this.configurationView.chooseDirectoryForDefaultVMPath()) {
				this.configurationModel.setDefault_virtual_machines_path(this.configurationView.getChoice());
			}
			this.configurationModel.setDefault_virtual_machines_option(true);
		} else if (e.getActionCommand().equals("OK")) {
			boolean do_my_view_invisible = true;
			if (this.configurationModel.isDefault_virtual_machines_option() && !this.checks_if_JTextField_is_changed(
					this.configurationView.getDefault_virtual_machines_path_choice(),
					this.configurationModel.getDefault_virtual_machines_path())) {
				if (this.configurationView.getChooser() != null) {
					this.configurationModel.setDefault_virtual_machines_path(
							this.configurationView.getDefault_virtual_machines_path_choice().getText());
				}
			}
			if (this.checks_if_JTextField_is_changed(this.configurationView.getDefault_virtual_machines_path_choice(),
					this.configurationModel.getDefault_virtual_machines_path())) {
				File file = new File(this.configurationView.getDefault_virtual_machines_path_choice().getText());
				if (file.isDirectory()) {
					this.configurationModel.setDefault_virtual_machines_path_with_boolean(
							this.configurationView.getDefault_virtual_machines_path_choice().getText());
				} else {
			...
			this.configurationModel
					.setExecute_before_start_qemu(this.configurationView.getExecute_before_start_qemu_choices());
			File file = new File(this.configurationView.getQemu_executable_path_choice().getText());
			if (file.isFile()) {
				this.configurationModel
						.setQemu_executable_path(this.configurationView.getQemu_executable_path_choice());
			} else {
				this.configurationView.showMessage("Please, reconfigure the path of the qemu executable first!\n"
			...
			this.configurationModel
					.setExecute_after_stop_qemu(this.configurationView.getExecute_after_stop_qemu_choices());
			File anotherfile = new File(this.configurationView.getQemu_img_executable_path_choice().getText());
			if (anotherfile.isFile()) {
				this.configurationModel
						.setQemu_img_executable_path(this.configurationView.getQemu_img_executable_path_choice());
			} else {
				this.configurationView.showMessage("Please, reconfigure the path of the qemu-img executable first!\n"
						+ "This path should be the path of a file!\n"
						+ "You can to cancel the configuration and the pending choices will be saved!");
				do_my_view_invisible = false;
			}

			if (this.configurationModel.isBios_vga_bios_keymaps_option() && !this.checks_if_JTextField_is_changed(
					this.configurationView.getBios_vga_bios_keymaps_path_choice(),
			...
			if (do_my_view_invisible) {
				...
				if (this.configurationView.getJavaQemu_configuration_file_path() != null) {
					this.fileControl.getMachineModel().setConfiguration(
							this.configurationView.getDefault_virtual_machines_path_choice(),
							this.configurationView.getExecute_before_start_qemu_choices(),
							this.configurationView.getQemu_executable_path_choice(),
							this.configurationView.getExecute_after_stop_qemu_choices(),
							...
							this.configurationView.getBios_vga_bios_keymaps_path_choice());
					this.fileControl.getMachineModel()
							.saveConfigurationToXML(this.configurationView.getJavaQemu_configuration_file_path());
				}
			...
			if (Model.isValidString(this.lastUsedFolderModel
					.getLastUsedFolder(LastUsedFolderEnumModel.SETQEMUEXECUTABLEDIRECTORY.getValor()))) {
				this.configurationView.setCurrentDirectory(this.lastUsedFolderModel
						.getLastUsedFolder(LastUsedFolderEnumModel.SETQEMUEXECUTABLEDIRECTORY.getValor()));
			}
			...
			this.configurationView.setCurrentDirectory(this.lastUsedFolderModel
					.getLastUsedFolder(LastUsedFolderEnumModel.SAVEEXISTINGJAVAQEMUCONFIGURATION.getValor()));
			if (this.configurationView.chooseConfigurationFileToBeSaved()) {
				this.lastUsedFolderModel.setLastUsedFolder(
						...
						(new File(this.configurationView.getJavaQemu_configuration_file_path())).getParent());
				this.fileControl.getMachineModel().setConfiguration(
						this.configurationView.getDefault_virtual_machines_path_choice(),
						this.configurationView.getExecute_before_start_qemu_choices(),
						this.configurationView.getQemu_executable_path_choice(),
			...
			if (this.configurationView.chooseConfigurationFileToBeOpened()) {
				this.lastUsedFolderModel.setLastUsedFolder(
						LastUsedFolderEnumModel.OPENEXISTINGJAVAQEMUCONFIGURATION.getValor(),
						(new File(this.configurationView.getJavaQemu_configuration_file_path())).getParent());
				this.lastUsedFileModel.setLastUsedFile(LastUsedFileEnumModel.LOADJAVAQEMUCONFIGURATIONFILE.getValor(),
						(new File(this.configurationView.getJavaQemu_configuration_file_path())).getName());
				this.fileControl.getMachineModel()
						.readConfigurationFromXML(this.configurationView.getJavaQemu_configuration_file_path());
				this.fileControl.getMachineModel().getConfiguration(
						this.configurationView.getDefault_virtual_machines_path_choice(),
						this.configurationView.getExecute_before_start_qemu_choices(),
						this.configurationView.getQemu_executable_path_choice(),
			...
			this.configurationView.setCurrentDirectory(
					this.lastUsedFolderModel.getLastUsedFolder(LastUsedFolderEnumModel.SETLOPTIONDIRECTORY.getValor()));
			...
			if (this.configurationView.chooseDirectoryForBiosVgaBiosKeymapsPath()) {
				this.configurationModel
						.setBios_vga_bios_keymaps_path(new JTextField(this.configurationView.getChoice()));
	...
	public void setVisibleEnable() {
		this.configurationView.setVisible(true);
	}

	public void setQemu_executable_path() {
		JTextField path = configurationView.getQemu_executable_path_choice();
		this.configurationModel.setQemu_executable_path(path);
	}

	public void setBios_vga_bios_keymaps_path() {
		JTextField path = this.configurationView.getBios_vga_bios_keymaps_path_choice();
		this.configurationModel.setBios_vga_bios_keymaps_path(path);
	}

	public boolean checks_if_JTextField_is_changed(JTextField textField, String string) {
