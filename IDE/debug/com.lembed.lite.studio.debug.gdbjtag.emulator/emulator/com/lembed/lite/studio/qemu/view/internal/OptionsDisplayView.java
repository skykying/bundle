...
public class OptionsDisplayView extends JFrame {
    ...
    public OptionsDisplayView(FileControl myfile) {
        ...
        if (myfile.getFilemodel().getDisplayType() != null) {
            this.displayType.setSelected(true);
            if (myfile.getFilemodel().getDisplayType().equals("sdl")) {
                this.displayTypeChoice.setSelectedIndex(0);
            } else if (myfile.getFilemodel().getDisplayType().equals("curses")) {
                this.displayTypeChoice.setSelectedIndex(1);
            } else if (myfile.getFilemodel().getDisplayType().equals("none")) {
                this.displayTypeChoice.setSelectedIndex(2);
            } else if (myfile.getFilemodel().getDisplayType().substring(0, 3).equals("vnc")) {
                this.displayTypeChoice.setSelectedIndex(3);
            }
        }

        if (myfile.getFilemodel().getNographicOption() != null) {
            if (myfile.getFilemodel().getNographicOption().equals("true")) {
                this.noGraphicOption.setSelected(true);
            }
        }

        if (myfile.getFilemodel().getVgaType() != null) {
            this.vgaType.setSelected(true);
            if (myfile.getFilemodel().getVgaType().equals("cirrus")) {
                this.vgaTypeChoice.setSelectedIndex(0);
            } else if (myfile.getFilemodel().getVgaType().equals("std")) {
                this.vgaTypeChoice.setSelectedIndex(1);
            } else if (myfile.getFilemodel().getVgaType().equals("vmware")) {
                this.vgaTypeChoice.setSelectedIndex(2);
            } else if (myfile.getFilemodel().getVgaType().equals("qxl")) {
                this.vgaTypeChoice.setSelectedIndex(3);
            } else if (myfile.getFilemodel().getVgaType().equals("none")) {
                this.vgaTypeChoice.setSelectedIndex(4);
            }
        }

        if (myfile.getFilemodel().getFullscreenOption() != null) {
            if (myfile.getFilemodel().getFullscreenOption().equals("true")) {
                this.fullScreenOption.setSelected(true);
            }
        }

        if (myfile.getFilemodel().getNoFrameOption() != null) {
            if (myfile.getFilemodel().getNoFrameOption().equals("true")) {
