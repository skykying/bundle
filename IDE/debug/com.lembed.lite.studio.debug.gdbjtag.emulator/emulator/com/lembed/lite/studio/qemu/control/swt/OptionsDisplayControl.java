...
import com.lembed.lite.studio.qemu.model.swt.options.OptionsDisplayModel;
...
public class OptionsDisplayControl implements BaseControl {
	...
	public OptionsDisplayControl(EmulationControl myemulation, EmulatorQemuMachineControl myfile) {
	...
	public void actionPerformed(ActionEvent e) {
		if (e.getActionCommand().equals("eraseButton")) {
			if (this.myview.getDisplayType().isSelected()) {
				this.mymodel
						.unsetOption(VMConfigurationModel.getTagsOptions()[OptionsEnumModel.DISPLAYOPTION.getValor()]);
			}
			if (this.myview.getNoGraphicOption().isSelected()) {
				this.mymodel.unsetOption(
						VMConfigurationModel.getTagsOptions()[OptionsEnumModel.NOGRAPHICOPTION.getValor()]);
			}
			...
			if (this.myview.getDisplayType().isSelected()) {
				if (this.myview.getDisplayTypeChoice().getSelectedItem().equals("sdl")) {
					this.mymodel.setDisplayTypeOption("sdl");
				} else if (this.myview.getDisplayTypeChoice().getSelectedItem().equals("curses")) {
					this.mymodel.setDisplayTypeOption("curses");
				} else if (this.myview.getDisplayTypeChoice().getSelectedItem().equals("none")) {
					this.mymodel.setDisplayTypeOption("none");
				} else if (this.myview.getDisplayTypeChoice().getSelectedItem().equals("vnc")) {
					StringBuilder result = new StringBuilder();

					if (!tcpPort.isEmpty()) {
						result.append("vnc=").append(ipAddress).append(":").append(tcpPort);
					} else {
			...
			if (this.myview.getNoGraphicOption().isSelected()) {
				this.mymodel
						.setOption(VMConfigurationModel.getTagsOptions()[OptionsEnumModel.NOGRAPHICOPTION.getValor()]);
			} else {
				this.mymodel.unsetOption(
						VMConfigurationModel.getTagsOptions()[OptionsEnumModel.NOGRAPHICOPTION.getValor()]);
			}
			...
			if (this.myview.getVgaType().isSelected()) {
				if (this.myview.getVgaTypeChoice().getSelectedItem().equals("Cirrus Logic GD5446")) {
					this.mymodel.setOption("-vga cirrus");
				} else if (this.myview.getVgaTypeChoice().getSelectedItem().equals("Standard VGA Card")) {
					this.mymodel.setOption("-vga std");
				} else if (this.myview.getVgaTypeChoice().getSelectedItem().equals("Vmware SVGA-II")) {
					this.mymodel.setOption("-vga vmware");
				} else if (this.myview.getVgaTypeChoice().getSelectedItem().equals("QXL Paravirtual VGA Card")) {
					this.mymodel.setOption("-vga qxl");
				} else if (this.myview.getVgaTypeChoice().getSelectedItem().equals("None")) {
			...
			if (!mysecondview.getiPAddressField().getText().isEmpty()) {
				if (!mysecondview.getiPAddressField().getText().equals(ipAddress)) {
