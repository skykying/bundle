...
public class VernacularViewer extends JPanel {
		...
		add(new JPanel() {
			...
			public void paintComponent(Graphics g) {
				...
				if (lastFrame != null) {
					...
					g2.drawImage(lastFrame, 0, 0, this.getWidth(), this.getHeight(), null);
				}
			}
		}, CENTER);
	}
	...
	private void initialiseVernacularClient() {
		...
		config.setRemoteClipboardListener(
				t -> getDefaultToolkit().getSystemClipboard().setContents(new StringSelection(t), null));
	...
	private boolean resizeRequired(Image frame) {
		return lastFrame == null || lastFrame.getWidth(null) != frame.getWidth(null)
	...
	private void resizeWindow(Image frame) {
		...
		int paddingTop = getHeight() - this.getHeight();
		int paddingSides = getWidth() - this.getWidth();
	...
	private int scaleMouseX(int x) {
		...
		return (int) (x * ((double) lastFrame.getWidth(null) / this.getWidth()));
	}
	...
	private int scaleMouseY(int y) {
		...
		return (int) (y * ((double) lastFrame.getHeight(null) / this.getHeight()));
	}
