 ...
 */
public class BuiltInToolsPathsPreferencePage extends FieldEditorPreferencePage implements IWorkbenchPreferencePage {

	// ------------------------------------------------------------------------

	/** The Constant ID. */
	public static final String ID = "com.lembed.lite.studio.managedbuild.cross.llvm.preferences.globalToolsPaths"; //$NON-NLS-1$

	// ------------------------------------------------------------------------

	/**
	 * Instantiates a new global tools paths preference page.
	 */
	public BuiltInToolsPathsPreferencePage() {
		super(GRID);

		setPreferenceStore(new ScopedPreferenceStore(ConfigurationScope.INSTANCE, LlvmUIPlugin.PLUGIN_ID));
	...
	public void init(IWorkbench workbench) {
	    ...
	    LlvmUIPlugin.log("GlobalToolsPathsPage.init()"); //$NON-NLS-1$
	}
		...
		for (int i = 0; i < projects.length; ++i) {
			...
			if (configs != null) {
				for (int j = 0; j < configs.length; ++j) {
					...
					IOption option = toolchain.getOptionBySuperClassId(ClangOption.OPTION_TOOLCHAIN_NAME);
					if (option == null) {
						continue;
					}
					try {
