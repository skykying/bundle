...
import com.lembed.lite.studio.managedbuild.cross.llvm.ui.ProjectStorage;
...
public class EnvironmentVariableSupplier implements IConfigurationEnvironmentVariableSupplier {
    ...
    public IBuildEnvironmentVariable getVariable(String variableName, IConfiguration configuration,
        ...
        LlvmUIPlugin.log("getVariable(" + variableName +"" + configuration.getName()+""); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
        return null;
	}
    ...
    public IBuildEnvironmentVariable[] getVariables(IConfiguration configuration,
		...
		LlvmUIPlugin.log("getVariables(" + configuration.getName() + ") returns empty array"); //$NON-NLS-1$ //$NON-NLS-2$
	...
	private static class PathEnvironmentVariable implements IBuildEnvironmentVariable {
		...
		private PathEnvironmentVariable(File path) {
		    LlvmUIPlugin.log("cpath=" + path); //$NON-NLS-1$
			this.path = path;
		}
		...
		public static PathEnvironmentVariable create(IConfiguration configuration) {
...

			{
				...
				if (isPathPerProject) {
					...
					LlvmUIPlugin.log("Path \"" + toolchainPath + "\" copied to project " + project.getName());					 //$NON-NLS-1$ //$NON-NLS-2$
				}
			}
			...
			LlvmUIPlugin.log("create(" + configuration.getName() + ") returns null"); //$NON-NLS-1$ //$NON-NLS-2$
			return null;
		}
		...
		private static String resolveMacros(String str, IConfiguration configuration) {
			...
			try {
			    ...
			    LlvmUIPlugin.log("resolveMacros " + e.getMessage()); //$NON-NLS-1$
			}
