...
public class DefaultPreferenceInitializer extends AbstractPreferenceInitializer {
	...
	public void initializeDefaultPreferences() {
		...
		LlvmUIPlugin.log("DefaultPreferenceInitializer.initializeDefaultPreferences()"); //$NON-NLS-1$
		...
		initializeDefaultPreferencesSecond();
	}
	
	   /**
   	 * Initialize default preferences second.
   	 */
   	public void initializeDefaultPreferencesSecond() {
	...
	private class LateInitializer implements INodeChangeListener {
		...
		public void added(NodeChangeEvent event) {
		    ...
		    LlvmUIPlugin.log("LateInitializer.added() " + event + " " + event.getChild().name());			 //$NON-NLS-1$ //$NON-NLS-2$

			if (LlvmUIPlugin.PLUGIN_ID.equals(event.getChild().name())) {
		...
		public void removed(NodeChangeEvent event) {			
		    LlvmUIPlugin.log("LateInitializer.removed() " + event);			 //$NON-NLS-1$
		}
		...
		public void finalizeInitializationsDefaultPreferences() {
			...
			for (ToolchainDefinition toolchain : ToolchainDefinition.getList()) {
				...
				if (!path.isEmpty()) {
				    LlvmUIPlugin.log(path + " | " + toolchainName); //$NON-NLS-1$
				...
				if (!searchPath.isEmpty()) {
					...
					if (value != null && !value.isEmpty()) {
						...
						LlvmUIPlugin.log(toolchainName +"()" + value);						 //$NON-NLS-1$
					}
