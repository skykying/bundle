...
import com.lembed.lite.studio.dataviewers.listeners.STDisposeListener;
import com.lembed.lite.studio.dataviewers.listeners.STHeaderListener;
...
public abstract class AbstractSTViewer {
    ...
    private void init(Composite parent, int style) {
        ...
        // (needs the columns to be created first)
        STDataViewersComparator scomparator = new STDataViewersComparator(getColumns());
        scomparator.restoreState(viewerSettings);
        setComparator(scomparator);
        setSortIndicators();
     ...
     *
     * @param comparatorUpdate
     */
    private void updateForNewComparator(STDataViewersComparator comparatorUpdate) {
        comparatorUpdate.saveState(viewerSettings);
        viewer.refresh();
        setSortIndicators();
    }
    ...
    public void saveState() {
        ...
        for (int i = 0; i < columnOrder.length; i++) {
            columnOrderStrings[i] = columnOrder[i] + ""; //$NON-NLS-1$
        }
    ...
    /**
     * Creates the column label provider.
     *
