 ...
 *    LiteSTUDIO - fix some bugs
 *******************************************************************************/
package com.lembed.lite.studio.dataviewers.charts.actions;
...
import com.lembed.lite.studio.internal.dataviewers.charts.Messages;
...
public class SaveChartAction extends Action {
    ...
    private static int getImageExtension(String imageExt) {
        if(!EXTENSION_MAP.containsKey(imageExt)) {
            return -1;
        }
        if(imageExt.equals("png")) { //$NON-NLS-1$
            return SWT.IMAGE_PNG;
        }
        if(imageExt.equals("bmp")) { //$NON-NLS-1$
            return SWT.IMAGE_BMP;
        }
        if(imageExt.equals("jpeg")) { //$NON-NLS-1$
            return SWT.IMAGE_JPEG;
        }
        if(imageExt.equals("jpg")) { //$NON-NLS-1$
            return SWT.IMAGE_JPEG;
        }
        return -1;
    }
    /**
     * Instantiates a new save chart action.
     */
    ...
    private static boolean shouldOverwrite(File file, Shell shell) {
        if (!file.exists()) {
            return true;
        }
     ...
     */
    private static String makePathWithVerifiedExt(String path) {
    ...
    private void generateImageFile(File file) throws IOException {
        // Extension is chosen based on the file name, not the dialog filter selection.
        String ext = Path.fromOSString(file.getName()).getFileExtension();
        if (ext == null) {
            throw new IOException();
        }
        
        int extension = getImageExtension(ext);
        if(extension >= 0) {
