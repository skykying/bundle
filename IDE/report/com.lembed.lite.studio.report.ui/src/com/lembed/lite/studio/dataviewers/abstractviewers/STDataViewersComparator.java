        ...
        if (priorities == null || directions == null) {
            ...
            if (priorities.length == columns.length && directions.length == columns.length) {
                ...
                STDataViewersActivator
                        .getDefault()
                        .getLog()
                        .log(new Status(IStatus.WARNING, STDataViewersActivator.PLUGIN_ID, "Invalid parameters:" //$NON-NLS-1$
                                + " priorities and/or directions number don't match with" //$NON-NLS-1$
                                + " viewer's columns count. Applying defaults settings.")); //$NON-NLS-1$
                resetState();
            }
        }
    }
