...
public class CompilerTableView {
    /**
     * 创建一个表格
     */
    public static TableViewer createTableViewer(Composite prarent){
        ...
        new TableColumn(table, SWT.NONE).setText("Description"); //$NON-NLS-1$

        tLayout.addColumnData(new ColumnWeightData(20));//这个是设置ID列的列宽为40像素
        new TableColumn(table, SWT.NONE).setText("Resource"); //$NON-NLS-1$
        
        tLayout.addColumnData(new ColumnWeightData(20));//这个是设置ID列的列宽为10像素
        new TableColumn(table, SWT.NONE).setText("Path"); //$NON-NLS-1$
    
        tLayout.addColumnData(new ColumnWeightData(20));//这个是设置ID列的列宽为10像素
        new TableColumn(table, SWT.NONE).setText("Location"); //$NON-NLS-1$
        
        tLayout.addColumnData(new ColumnWeightData(40));//这个是设置ID列的列宽为70像素
        new TableColumn(table, SWT.NONE).setText("Type"); //$NON-NLS-1$
        
        tableViewer.setContentProvider(new TableViewerContentProvider());
        //5.设定标签器
        tableViewer.setLabelProvider(new TableViewerLabelProvider());
        //6.用setInput输入数据(把PeopleFactory产生的List集合传进来)
        tableViewer.setInput(BuildFactory.getEmpty());
    ...
    public static void addListener(TableViewer tableViewer){
        ...
        tableViewer.addDoubleClickListener(new IDoubleClickListener() {//IDoubleClickListener是一个接口
            ...
            public void doubleClick(DoubleClickEvent event) {
                event.getSelection();
                MessageDialog.openInformation(null, "INFO", "name"); //$NON-NLS-1$ //$NON-NLS-2$
            }
        });
