...
public class BtfEventType extends TmfEventType {
    ...
    public ITmfEventField generateContent(String event, long sourceInstance, long targetInstance) {
        ...
        if (fHasNotes) {
            ...
            TmfEventField eventField = new TmfEventField(BtfColumnNames.EVENT.toString(), data[0].trim(), BTFPayload.getFieldDescription(data[0]));
            TmfEventField notesField = new TmfEventField(BtfColumnNames.NOTES.toString(), data.length == 2 ? data[1].trim() : null, null);
            ...
            TmfEventField eventField = new TmfEventField(BtfColumnNames.EVENT.toString(), data[0].trim(), BTFPayload.getFieldDescription(data[0].trim()));
