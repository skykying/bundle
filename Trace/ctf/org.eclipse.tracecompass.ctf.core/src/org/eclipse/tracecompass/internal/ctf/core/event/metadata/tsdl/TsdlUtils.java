...
public final class TsdlUtils {
     ...
     */
    public static @NonNull String concatenateUnaryStrings(List<? extends Object> strings) throws ParseException {

        StringBuilder sb = new StringBuilder();

        CommonTree first = (CommonTree) strings.get(0);
        sb.append(STRING_PARSER.parse(first, null));

        boolean isFirst = true;
        List<? extends CommonTree> trees = (List<? extends CommonTree>) strings;
        for (CommonTree ref : trees) {
            if (isFirst) {
                isFirst = false;
                continue;
            }
