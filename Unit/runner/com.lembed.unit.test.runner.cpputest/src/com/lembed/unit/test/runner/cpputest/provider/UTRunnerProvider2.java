...
import com.lembed.unit.test.runner.cpputest.handler.OutputHandler;
 ...
 * Runner Plug-In.
 */
public class UTRunnerProvider2 implements ITestsRunnerProvider {
	...
	private String getErrorText(String prefix, String description) {
		return MessageFormat.format(Messages.UnitTestsRunner_error_format, prefix, description);
	}
	...
	public void run(ITestModelUpdater modelUpdater, InputStream inputStream) throws TestingException {
		...
		try {
			...
			throw new TestingException(getErrorText(Messages.UnitTestsRunner_io_error_prefix, e.getLocalizedMessage()));
		}
	}
